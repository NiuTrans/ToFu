...
pytest.importorskip('lib.agent_core.task_runtime', reason='needs Tofu 0.16 host')
...
def test_create_returns_task_id():
    from tofu_trading.web.handlers.trading_simulator import _create_task
    tid = _create_task('fetch', owner_user_id=USER_ID)
...
def test_create_distinct_kinds():
    from tofu_trading.web.handlers.trading_simulator import _create_task
    t1 = _create_task('fetch', owner_user_id=USER_ID)
    t2 = _create_task('sim', owner_user_id=USER_ID)
    assert t1 != t2
    _ok('_create_task() returns unique IDs for fetch + sim')


def test_append_and_poll_returns_events():
    from tofu_trading.web.handlers.trading_simulator import _create_task, _append_event, _get_task_progress
    tid = _create_task('fetch', owner_user_id=USER_ID)
    _append_event(tid, {'phase': 'prices', 'done': 1, 'total': 10})
    _append_event(tid, {'phase': 'prices', 'done': 5, 'total': 10})

    resp = _get_task_progress(tid, cursor=0, owner_user_id=USER_ID)
...
def test_poll_cursor_replay():
    from tofu_trading.web.handlers.trading_simulator import _create_task, _append_event, _get_task_progress
    tid = _create_task('sim', owner_user_id=USER_ID)
    for i in range(5):
        _append_event(tid, {'i': i})

    r0 = _get_task_progress(tid, cursor=0, owner_user_id=USER_ID)
    assert len(r0['events']) == 5
    assert r0['cursor'] == 5

    _append_event(tid, {'i': 5})
    r1 = _get_task_progress(tid, cursor=5, owner_user_id=USER_ID)
...
def test_finish_with_result():
    from tofu_trading.web.handlers.trading_simulator import _create_task, _append_event, _finish_task, _get_task_progress
    tid = _create_task('fetch', owner_user_id=USER_ID)
...
def test_finish_with_string_error():
    from tofu_trading.web.handlers.trading_simulator import _create_task, _finish_task, _get_task_progress
    tid = _create_task('sim', owner_user_id=USER_ID)
...
def test_finish_with_dict_envelope():
    from tofu_trading.web.handlers.trading_simulator import _create_task, _finish_task, _get_task_progress
    tid = _create_task('fetch', owner_user_id=USER_ID)
    envelope = {'kind': 'ratelimit', 'detail': 'too many requests',
                'source': 'test'}
    _finish_task(tid, error=envelope)

    resp = _get_task_progress(tid, cursor=0, owner_user_id=USER_ID)
    assert resp['error']['kind'] == 'ratelimit'
...
def test_unknown_task_returns_legacy_string_error():
    from tofu_trading.web.handlers.trading_simulator import _get_task_progress
    resp = _get_task_progress(
        'does-not-exist', cursor=0, owner_user_id=USER_ID)
...
def test_http_endpoints_via_test_client():
    ...
    async def _test():
        async with app.test_client() as client:
            from tofu_trading.web.handlers.trading_simulator import _create_task, _append_event, _finish_task
            ...
            tid2 = _create_task('sim', owner_user_id=USER_ID)
