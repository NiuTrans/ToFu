...
class TaskState:
    ...
    __slots__ = ('task_id', 'task_type', 'status', 'params',
                 ...
                 'owner_user_id', '_cancel_flag')

    def __init__(self, task_id, task_type, params=None, *, owner_user_id):
...
def submit_task(task_type, run_fn, params=None, *, owner_user_id):
    ...
    task = TaskState(
        task_id, task_type, params, owner_user_id=owner_user_id)
...
def get_task(task_id, *, owner_user_id):
    ...
    return _load_from_db(task_id, owner_user_id=owner_user_id)


def poll_task(task_id, cursor=0, *, owner_user_id):
    ...
    """
    task = get_task(task_id, owner_user_id=owner_user_id)
    if not task:
        return None
...
def cancel_task(task_id, *, owner_user_id):
...
def list_active_tasks(task_type=None, *, owner_user_id):
...
def _persist_task(task):
    ...
    try:
        ...
        db = get_thread_db(
            DOMAIN_TRADING, owner_user_id=task.owner_user_id)
