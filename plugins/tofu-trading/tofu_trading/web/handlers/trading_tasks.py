...
from tofu_trading.trading_tasks import cancel_task, get_task, list_active_tasks, poll_task, submit_task
from tofu_trading.storage import DOMAIN_TRADING, get_db, get_thread_db

logger = get_logger(__name__)

from tofu_trading.web.v1.tasks import api_v1_trading_tasks_bp as trading_tasks_bp  # noqa: E402
# (alias kept for back-compat with `from tofu_trading.web.handlers.trading_tasks import trading_tasks_bp` callers)
...
async def tasks_submit():
    ...
    # The _submit_* helpers gather context with blocking sync DB calls and then
    # spawn a background task — offload the whole call so the event loop stays
    # responsive. asyncio.to_thread copies the current context, so jsonify()
    # still has the request/app context available inside the worker thread.
    if task_type == 'decision':
        return await asyncio.to_thread(_submit_decision, params)
    elif task_type == 'autopilot':
        return await asyncio.to_thread(_submit_autopilot, params)
    elif task_type == 'intel_backtest':
        return await asyncio.to_thread(_submit_intel_backtest, params)
    else:
...
async def tasks_poll(task_id):
    ...
    result = await asyncio.to_thread(
        poll_task, task_id, cursor, owner_user_id=uid)
    if result is None:
...
async def tasks_result(task_id):
    """Get final result of a completed task."""
    import asyncio
    uid = current_user_id()
    task = await asyncio.to_thread(
        get_task, task_id, owner_user_id=uid)
    if not task:
...
async def tasks_cancel(task_id):
    """Cancel a running task."""
    import asyncio
    uid = current_user_id()
    ok = await asyncio.to_thread(
        cancel_task, task_id, owner_user_id=uid)
    if ok:
...
async def tasks_active():
    ...
    tasks = await asyncio.to_thread(
        list_active_tasks, task_type, owner_user_id=uid)
...
def _submit_decision(data):
    ...
    from tofu_trading.trading import calc_sell_fee, fetch_trading_fees
    from tofu_trading.trading.news_gathering import gather_news_cached as _gather_news_cached
    from tofu_trading.web.handlers.trading_decision import _auto_save_strategies, _extract_and_queue_trades
    from tofu_trading.web.handlers.trading_intel import _get_holdings_ctx, _get_strategies_ctx

    from tofu_trading.identity import current_user_id
    # Resolve the user HERE, in the request thread. The background worker below
    # has no request context, so it must close over this value rather than
    # re-resolving it (which would raise, or worse, resolve to the wrong user).
    uid = current_user_id()

    db = get_db(DOMAIN_TRADING, owner_user_id=uid)
    holdings_ctx = _get_holdings_ctx(db, uid)
    ...
    strategies_ctx = _get_strategies_ctx(db, uid)
    ...
    if group_id:
        ...
        if grp:
            ...
            if sids:
                ph = ','.join('?' * len(sids))
                gstrats = [dict(r) for r in db.execute(f'SELECT * FROM trading_strategies WHERE id IN ({ph}) AND user_id=?', (*sids, uid)).fetchall()]
    ...
    fee_ctx = "\n## 费率信息\n"
    for h_row in db.execute('SELECT * FROM trading_holdings WHERE user_id=?', (uid,)).fetchall():
        h = dict(h_row)
...
请深度思考后给出专业、有依据的建议。使用 Markdown 格式。"""
    ...
    def run(task):
        ...
        try:
            _db = get_thread_db(DOMAIN_TRADING, owner_user_id=uid)
            today = datetime.now().strftime('%Y-%m-%d')
            now_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            _db.execute('INSERT OR REPLACE INTO trading_daily_briefing (user_id,date,content,news_json,created_at) VALUES (?,?,?,?,?)',
                        (uid, today, task.result, json.dumps(_news, ensure_ascii=False), now_str))
            _db.commit()
            _extract_and_queue_trades(_db, task.result, uid)
            _auto_save_strategies(_db, task.result, uid)
        except Exception as e:
...
def _submit_autopilot(data):
    ...
    from lib.llm_dispatch import dispatch_stream
    from tofu_trading.trading_autopilot import (
    ...
    body, context = build_autopilot_streaming_body(
        db, news_items=news, cycle_number=cycle_number, uid=uid)
        ...
        try:
            ...
            try:
                ...
                _store_cycle_result(
                    ...
                    parsed, uid, _context.get('kpi_evaluations', {}),
                    _context.get('correlations', [])
                )
                _apply_strategy_updates(_db, parsed.get('strategy_updates', []), uid)
                _db.commit()
...
def _submit_intel_backtest(data):
    ...
    if sids:
        ph = ','.join('?' * len(sids))
        strategies = [dict(r) for r in db.execute(f'SELECT * FROM trading_strategies WHERE id IN ({ph}) AND user_id=?', (*sids, uid)).fetchall()]
