...
from tofu_trading.trading_autopilot._constants import AUTOPILOT_CYCLE_MINUTES
...
def autopilot_scheduler_tick(
    ...
    *,
    ...
    Args:
        ...
        client:         Optional :class:`~tofu_trading.trading._common.TradingClient` instance
    ...
    try:
        ...
        from tofu_trading.trading_autopilot.cycle import run_autopilot_cycle

        _autopilot_state['cycle_count'] += 1
        # Background scheduler thread: no request context, so the owner cannot
        # be resolved per-request. The autopilot runs on behalf of the default
        # owner until per-user scheduling exists (P1) -- being explicit here
        # beats silently inheriting whichever rows happen to be in the table.
