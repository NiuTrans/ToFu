...
def detect_market_condition(
    ...
    try:
        from tofu_trading.trading.intel_timeline import build_regime_intel_features
...
def select_strategies(
    ...
    *,
    ...
    strategies = db.execute(
        "SELECT * FROM trading_strategies WHERE status='active' AND user_id=? ORDER BY updated_at DESC",
        (uid,)
    ).fetchall()
    ...
    #
    # strategy_learner / strategy_evolution were DELETED (REDESIGN P3, ~4,400
    # lines of never-executed code). These blocks used to swallow the resulting
    # ImportError at warning level, so selection silently ran with no learning
    # data at all while the UI showed a healthy strategy engine — a feature that
    # is unavailable but presents as working.
    #
    # ModuleNotFoundError is now separated from genuine runtime failures: a
    # missing module is a KNOWN, permanent state reported once at INFO, while an
    # unexpected error still surfaces at ERROR with a traceback. Either way the
    # degradation is recorded on the result so callers can see it.
    compatibility: dict = {}
    failures: dict = {}
    degraded: list[str] = []
    try:
        from tofu_trading.trading_autopilot.strategy_learner import (
        ...
        degraded.append('strategy_learner')
        logger.info('[MetaStrategy] strategy_learner not installed (removed in P3) '
                    '— selection runs WITHOUT compatibility/failure data')
    except Exception as e:
        degraded.append('strategy_learner')
        logger.error('[MetaStrategy] strategy_learner failed unexpectedly: %s', e,
                     exc_info=True)

    # ── Load historical performance ──
    perf_cache: dict[int, float] = {}
    try:
        from tofu_trading.trading_autopilot.strategy_evolution import evaluate_strategy_history
        for s in strategies:
        ...
        degraded.append('strategy_evolution')
        logger.info('[MetaStrategy] strategy_evolution not installed (removed in P3) '
                    '— selection runs WITHOUT historical win-rate data')
    except Exception as e:
        degraded.append('strategy_evolution')
        logger.error('[MetaStrategy] strategy_evolution failed unexpectedly: %s', e,
                     exc_info=True)
