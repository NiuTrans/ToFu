...
def compute_metrics(state, bm_lookup, all_dates, initial_capital):
    ...
    # ── Win rate ──────────────────────────────────────────
    # A winning sell = net proceeds (after fees) exceed the cost basis of the
    # shares sold, i.e. the realized trade made money. ``cost_basis`` is
    # recorded by the engine on each sell; older trade logs without it fall
    # back to gross amount (best-effort, flags any profitable-looking exit).
    sell_trades = [t for t in state.trade_log if t['type'] == 'sell']
    winning_trades = [
        t for t in sell_trades
        if t.get('net_proceeds', t.get('amount', 0)) > t.get('cost_basis', t.get('amount', 0))
    ]
