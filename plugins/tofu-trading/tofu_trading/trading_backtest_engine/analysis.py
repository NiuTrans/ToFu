...
def analyze_transaction_cost_impact(asset_prices, benchmark_navs=None, config=None):
    ...
    # Run without costs
    # Run without costs.
    # One switch, not three zeroed rate keys: the old form silently missed any
    # fee component the caller forgot to list, and the component list is now
    # per-asset-type (commission / stamp tax / transfer fee / tiered redemption).
    no_cost_config = dict(config)
    no_cost_config['zero_cost'] = True
...
def run_walk_forward(asset_prices, config=None, n_splits=5, **kwargs):
    ...
    """
    from tofu_trading.trading_strategy_engine import rolling_walk_forward_optimize
