...
class BacktestEngine(StrategyMixin):
    ...
    def __init__(self, config=None):
        ...
        # Fees resolve through the shared FeeBook (single source of truth,
        # per asset type and per trade size). zero_cost drives the cost-free
        # control run in analysis.analyze_transaction_cost_impact.
        self.fee_book = _cfg.get('fee_book') or FeeBook(
            zero_cost=bool(_cfg.get('zero_cost')))
    ...
    def _place_buy_order(self, state, code, amount, date, reason, stop_mgr, nav, risk_params):
        ...
        fee = self.fee_book.fee_for(code, amount, 'buy')['fee_amount']
    ...
    def _execute_pending_orders(self, state, nav_lookup, date, stop_mgr):
        ...
        for order in state.pending_orders:
            ...
            if nav is None or nav <= 0:
                # Can't execute today — carry forward, or cancel after 3
                # trading days spent waiting for a valid NAV. days_waiting
                # counts trading days observed strictly AFTER placement (the
                # prior metric counted days BEFORE placement, so early orders
                # never expired and late ones expired immediately).
                days_waiting = len([d for d in state.daily_values if d['date'] > placed])
                if days_waiting > 3:
                    # Cancel the stale order. Buys reserved cash up-front in
                    # _place_buy_order, so refund amount+fee; sells reserved
                    # no cash, so they are simply dropped.
                    if order['type'] == 'buy':
            ...
            if order['type'] == 'buy':
                ...
                # fee_book. For exchange-traded assets it is ignored, which is
                # correct: A-shares have no short-holding penalty.
                fee = self.fee_book.fee_for(
