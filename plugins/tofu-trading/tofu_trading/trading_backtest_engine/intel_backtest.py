...
from tofu_trading.trading.intel_timeline import (
...
_ADAPTIVE_ENGINE_STATE: dict = {}


def _adaptive_engine_available() -> bool:
    """Whether AdaptiveDecisionEngine is installed. Announced exactly once.

    The module was removed in REDESIGN P3. Its absence is a permanent property
    of the installation, so it is resolved once per process and reported at
    WARNING — loudly enough that nobody mistakes matrix-only selection for the
    full learning-backed engine, quietly enough not to emit a traceback per
    simulated day.
    """
    if 'available' not in _ADAPTIVE_ENGINE_STATE:
        try:
            import tofu_trading.trading_autopilot.adaptive_decision_engine  # noqa: F401
            _ADAPTIVE_ENGINE_STATE['available'] = True
        except ModuleNotFoundError:
            _ADAPTIVE_ENGINE_STATE['available'] = False
            logger.warning(
                '[IntelBacktest] adaptive_decision_engine is NOT installed '
                '(removed in REDESIGN P3). Strategy selection DEGRADES to the raw '
                'suitability matrix: no learning data, no compatibility checks, '
                'no failure restrictions. Results are not comparable to a run '
                'with the engine present.')
    return _ADAPTIVE_ENGINE_STATE['available']
...
class IntelBacktestConfig:
    ...
    def __init__(self, **kwargs):
        ...
        # Fees resolve through the shared FeeBook (see config.py for why the
        # old rate keys and short_sell_penalty were removed rather than moved).
...
class IntelBacktestEngine:
    ...
    def run(
        ...
        for day_idx, current_date in enumerate(all_dates):
            ...
            if cfg.intel_enabled and cfg.meta_strategy_enabled:
                ...
                #
                # adaptive_decision_engine was DELETED (REDESIGN P3). Resolution
                # is hoisted out of the per-date loop deliberately: the old form
                # raised ImportError on EVERY decision date and logged it at
                # warning WITH a traceback, so a permanent, known condition
                # produced one stack trace per simulated day. Availability is a
                # property of the installation, not of the date being simulated.
                if _adaptive_engine_available():
                    try:
                        from tofu_trading.trading_autopilot.adaptive_decision_engine import (
                        ...
                        logger.warning('[IntelBacktest] AdaptiveDecisionEngine failed at %s, '
                                       'falling back to matrix selection: %s', current_date, e,
                                       exc_info=True)
                    ...
                    # Documented, permanent degradation — announced once by
                    # _adaptive_engine_available(), not once per simulated day.
                    market_condition = self._detect_condition(
                        signals, intel_features, current_date,
                    )
                    selected_strategy_types = self._select_strategy_types(
    ...
    def _place_buy_order(self, state, code, amount, date, reason, stop_mgr, nav, risk_params):
        ...
        fee = cfg.fee_book.fee_for(code, amount, 'buy')['fee_amount']
        state.cash -= amount
    ...
    def _execute_pending_orders(self, state, nav_lookup, date, stop_mgr):
        ...
        for order in state.pending_orders:
            ...
            if order['type'] == 'buy':
                ...
                # Holding period feeds the fund tiered-redemption table inside
                # fee_book; exchange-traded assets ignore it, which is correct.
                fee = cfg.fee_book.fee_for(
                    code, gross_amount, 'sell', holding_days=hold_days)['fee_amount']
