...
(function (F) {
  ...
  //
  //  The landing page ships ordered for a stranger: hero → how-it-works →
  //  track record → … → the user's own holdings last. That is correct
  //  exactly once. This is a daily-return tool, so from the second visit
  //  the explainer stands between the user and the only thing they opened
  //  the page for. These two helpers re-rank the front screen once the
  //  holdings answer is known (it is fetched on every load anyway).
  // ══════════════════════════════════════════

  /** Move the holdings block above the onboarding blocks. */
  function _promoteHoldingsFirst(section) {
    if (!section || !section.parentNode) return;
    var first = section.parentNode.firstElementChild;
    if (first && first !== section) section.parentNode.insertBefore(section, first);
  }

  /** Show/hide the first-run onboarding (hero copy + the 3-step explainer). */
  function _setOnboardingVisible(visible) {
    ['.ov-hero', '.ov-howto'].forEach(function (sel) {
      var el = document.querySelector(sel);
      if (el) el.style.display = visible ? '' : 'none';
    });
  }

  /** Restore the shipped document order (holdings back to the bottom). */
  function _restoreHoldingsLast(section) {
    if (!section || !section.parentNode) return;
  ...
  async function _loadPortfolioPeek() {
    try {
      ...
      // Both branches must be EXPLICIT. loadOverview() re-runs on every
      // return to the tab, so a bare `return` here would leave whatever the
      // previous visit did in place: a user who sells everything would keep
      // a stale portfolio box and never get the onboarding back.
      if (holdings.length === 0 && cash === 0) {
        if (section) section.style.display = 'none';
        _restoreHoldingsLast(section);
        _setOnboardingVisible(true);
        return;
      }

      if (section) section.style.display = '';
      _promoteHoldingsFirst(section);
