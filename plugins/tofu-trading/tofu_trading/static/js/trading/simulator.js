...
(function (F) {
  ...
  function _doSymbolSearch(keyword) {
    ...
    window.Api.trading.raw('/sim/search?q=' + encodeURIComponent(keyword))
  ...
  function startFetchData() {
    ...
    window.Api.trading.raw('/sim/fetch-data', {
      method: 'POST',
      body: body,
  ...
  function _startFetchPoll(taskId, cursor) {
    var pollPath = '/sim/fetch-progress/' + taskId;
    var _cursor = cursor;

    _activeFetchPoll = setInterval(function () {
      window.Api.trading.raw(pollPath + '?cursor=' + _cursor)
        .then(function (r) { return r.json(); })
        .then(function (resp) {
  ...
  function startSimulation() {
    ...
    window.Api.trading.raw('/sim/run', { method: 'POST', body: body })
    .then(function (resp) { return resp.json(); })
    .then(function (data) {
      if (data.error) {
        var _msg = (typeof errorEnvelopeMessage === 'function')
  ...
  function _startSimPoll(taskId, cursor) {
    var pollPath = '/sim/run-progress/' + taskId;
    var _cursor = cursor;
  ...
  // Resolve the current theme's chart palette from the live CSS variables.
  // Reads getComputedStyle each render, so it follows a theme change without
  // a reload. Falls back to the dark palette if tokens are unavailable.
  function _chartTheme() {
    ...
    // --bg2 / --success / --danger / --t2 are plain hex/rgb literals in the
    // bridge, so getComputedStyle resolves them directly. color-mix() values
    // would come back unresolvable, which is why we only read plain tokens.
    var up = tok('--success', '#00E59B');
    var down = tok('--danger', '#FF4D6A');
    return {
      panel: tok('--bg2', '#111115'),
      grid: _alpha(tok('--t2', '#8b95b0'), 0.25),
      up: up,
      down: down,
      upFill: function (isUp) { return _alpha(isUp ? up : down, 0.15); },
    };
  }

  // Expand #rgb / #rrggbb / rgba(...) into rgba(...) at the given alpha.
  function _alpha(color, a) {
    var m = color.match(/^#([0-9a-fA-F]{3,8})$/);
    if (m) {
      var hex = m[1];
    ...
    var m2 = color.match(/^rgba?\(([^)]+)\)$/);
    if (m2) {
      var parts = m2[1].split(',').slice(0, 3).join(',');
      return 'rgba(' + parts + ',' + a + ')';
    }
    return color;
  }
  ...
  function _renderMiniEquity(points, initialCapital) {
    ...
    // Canvas cannot read CSS variables, so pull the CURRENT theme's tokens
    // from getComputedStyle at render time. The old code hardcoded a dark
    // plate + white-glass baseline + neon equity colours: on a light theme
    // the baseline vanished and the panel read as a stray dark slab.
    var tk = _chartTheme();
    ...
    var color = lastVal >= initialCapital ? tk.up : tk.down;
    ...
    grad.addColorStop(0, tk.upFill(lastVal >= initialCapital));
