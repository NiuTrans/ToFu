  ...
  function renderHoldings() {
    ...
    tbody.innerHTML = S.holdings
      .map((h) => {
        ...
        return `
      <tr class="${rowCls}">
        <td class="code cell-primary" data-label="代码">${h.symbol}</td>
        <td data-label="名称">${escHtml(h.asset_name || "--")}</td>
        <td class="num" data-label="份额">${fmtNum(h.shares, 2)}</td>
        <td class="num" data-label="成本价">¥${fmtNum(h.buy_price, 4)}</td>
        <td class="num" data-label="现价">${h.current_nav ? "¥" + fmtNum(h.current_nav, 4) : "--"}${h.nav_date ? `<small class="nav-date">${h.nav_date}</small>` : ""}</td>
        <td class="num" data-label="估算">${h.est_nav ? "¥" + fmtNum(h.est_nav, 4) : "--"}</td>
        <td class="num" data-label="市值">¥${fmtNum(marketValue)}</td>
        <td class="num ${pnlClass(holdingPnl)}" data-label="盈亏">
          ${h.current_nav ? (holdingPnl >= 0 ? "赚了 " : "亏了 ") + "¥" + fmtNum(Math.abs(holdingPnl)) : "--"}
        </td>
        <td class="num ${pnlClass(holdingPnl)}" data-label="盈亏比例">
