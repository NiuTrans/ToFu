...
def _realised_sigma(db, symbol: str, as_of: str, lookback_days: int = 60) -> float | None:
    """Realised per-bar volatility from bars STRICTLY up to ``as_of``.

    Returns the standard deviation of log returns per bar, or None when there is
    not enough history. Uses only ``date <= as_of``, so it carries no look-ahead.
    """
    try:
        start = (datetime.strptime(as_of, '%Y-%m-%d')
                 - timedelta(days=lookback_days * 2)).strftime('%Y-%m-%d')
    except (ValueError, TypeError) as e:
        logger.debug('[Sim] sigma date parse failed for %s: %s', as_of, e)
        return None

    rows = db.execute(
        'SELECT nav FROM trading_sim_prices WHERE symbol=? AND date>=? AND date<=? '
        'ORDER BY date', (symbol, start, as_of)
    ).fetchall()
    navs = [float(dict(r)['nav']) for r in rows if dict(r).get('nav')]
    if len(navs) < 20:
        return None
...
def _adaptive_stop_pct(db, symbol: str, as_of: str, holding_bars: int,
                       config) -> tuple[float, str]:
    """Stop-loss threshold scaled to the volatility of the ACTUAL holding period.

    WHY A FIXED PERCENTAGE IS THE WRONG SHAPE
    -----------------------------------------
    A flat 5% stop is not a constant amount of risk — how likely it is to be hit
    depends entirely on how volatile the asset is and how long the position is
    held. Measured against the default 5-trading-day decision cadence:
    ...
    Below ~2 sigma the stop is inside the asset's ordinary noise band, so it
    mostly fires on random wiggles rather than on genuine adverse moves — each
    firing paying a full round-trip of fees and re-entering at a worse price.
    For growth names, at 0.79 sigma, that is the common case rather than the
    exception (measured ~10.8 noise exits per year per position).

    So the threshold is expressed where the meaning actually lives: in sigma of
    the holding period. sigma is measured from realised history up to ``as_of``
    only, and scaled by sqrt(holding bars) — a position held longer has had more
    chances to wander, and its stop must widen accordingly or it becomes a
    guaranteed exit.

    Args:
        db:           Database connection.
        symbol:       Asset code.
        as_of:        Evaluation date (no data after this is read).
        holding_bars: Bars held so far; at least 1.
        config:       SimulatorConfig — supplies stop_loss_sigma, the floor/cap,
                      and the fixed fallback.

    Returns:
        ``(threshold_pct, basis)`` where basis is 'adaptive' or 'fixed'
        (the latter when history is too short to measure sigma).
    """
    if not getattr(config, 'adaptive_stop', True):
        return abs(config.stop_loss_pct), 'fixed'

    sigma = _realised_sigma(db, symbol, as_of)
    if sigma is None:
        return abs(config.stop_loss_pct), 'fixed'

    horizon = max(int(holding_bars), 1)
    pct = config.stop_loss_sigma * sigma * math.sqrt(horizon) * 100.0
    # Clamp: a floor keeps a dead-quiet asset from being stopped on rounding,
    # a cap keeps a wild one from having no stop at all.
    pct = max(config.min_stop_pct, min(pct, config.max_stop_pct))
    return pct, 'adaptive'


def _next_bar_fill(db, symbol: str, decision_date: str, end_date: str):
    """Resolve the T+1 OPEN price at which a decision made on T is executed.

    WHY THIS EXISTS — the look-ahead this removes
    ---------------------------------------------
    Signals are computed from bars with ``date <= decision_date`` (i.e. they see
    T's CLOSE), and execution used ``get_price_at(db, symbol, decision_date)``,
    which is that very same close. The simulator was therefore deciding at T's
    close and filling at T's close — "look at today's close, then buy at today's
    close". No real participant can do that: by the time T's close is known, the
    only tradeable price left is T+1's open.

    Direction of the effect: this makes results WORSE, and that is the point.
    Measured zero-alpha baseline under same-bar fills was +1.258% (ETF) and
    +1.016% (stock) — positive returns from a driftless random walk, which is
    the signature of the bias. Honest fills must push those toward the negative
    cost floor.

    NO SILENT FALLBACK
    ------------------
    When T+1 has no bar (suspension, or the decision lands on the final bar of
    the window) this returns ``(None, reason)`` and the caller MUST skip the
    trade. Falling back to T's close would quietly reinstate the very bias this
    function exists to remove, while looking like a harmless safety net.

    Args:
        db:            Database connection.
        symbol:        Asset code.
        decision_date: The date the decision was made (T).
        end_date:      Simulation end; a fill beyond it is not executable.

    Returns:
        ``(price, None)`` on success, or ``(None, reason)`` when unfillable.
    """
    row = db.execute(
        'SELECT date, open, nav FROM trading_sim_prices '
        'WHERE symbol=? AND date>? AND date<=? ORDER BY date LIMIT 1',
        (symbol, decision_date, end_date)
    ).fetchone()
    if not row:
        return None, f'T+1无行情（{decision_date} 之后至 {end_date} 无可成交的交易日）'

    row = dict(row)
    price = row.get('open') or 0
    if price <= 0:
        # Open is absent for this bar. Using its CLOSE would be a different bar's
        # close, not a look-ahead, but it is still not the price an order placed
        # at T would have filled at — so it is reported, never silently used.
        return None, (f"T+1（{row['date']}）无开盘价，不以收盘价代替成交")
    return float(price), None


def _describe_fees(fee_book, symbols, sample_amount: float) -> str:
    """Render the fee schedule the LEDGER will actually charge, for the prompt.

    WHY THIS EXISTS
    ---------------
    The prompt used to state one fee schedule while the ledger charged another.
    The system prompt told the model "commission ~0.025% + 0.05% stamp tax"
    (0.1% round-trip) while the ledger deducted 0.15%/0.5% (0.65% round-trip) —
    6.5x more. The model sized trades and judged turnover against a cost that
    was never applied, and the same prompt urged it to trade individual stocks
    actively. It was being penalised for following its own instructions.

    So both prompt sites and the ledger now derive from ONE source: this
    function reads the same FeeBook that compute_fee bills against. A rate can
    no longer drift between what the model is told and what it is charged.

    Args:
        fee_book:      The run's FeeBook — the same object the ledger bills from.
        symbols:       Symbols to describe.
        sample_amount: Trade size used to render effective rates. Required
                       because the ¥5 commission floor makes the rate
                       size-dependent (¥500 → 1.000%, ¥50,000 → 0.025%).

    Returns:
        Markdown lines describing real round-trip cost per asset type.
    """
    by_type: dict[str, str] = {}
    for symbol in symbols:
        sched = fee_book.schedule_for(symbol)
        if sched.asset_type in by_type:
            continue
        buy = fee_book.fee_for(symbol, sample_amount, 'buy')
        sell = fee_book.fee_for(symbol, sample_amount, 'sell', holding_days=30)
        rt = (buy['fee_rate'] + sell['fee_rate']) * 100
        label = {'stock': '个股', 'etf': 'ETF', 'bond': '债券',
                 'fund': '场外基金'}.get(sched.asset_type, sched.asset_type)
        line = (f"- {label}: 买入 {buy['fee_rate']*100:.4f}% + 卖出 "
                f"{sell['fee_rate']*100:.4f}% = 往返 {rt:.4f}%")
        if sched.asset_type == 'stock':
            line += '（卖出含 0.05% 印花税）'
        elif sched.asset_type == 'fund':
            line += '（持有 <7 天赎回费高达 1.5%，持有越久越低）'
        if sched.min_commission > 0:
            line += f"，佣金最低 ¥{sched.min_commission:g}"
        by_type[sched.asset_type] = line

    if not by_type:
        return '- （本轮无持仓标的）'
    note = (f"\n（以上为单笔 ¥{sample_amount:,.0f} 的有效费率。"
            f"佣金有最低收取额，所以小额交易的实际费率更高）")
    return '\n'.join(by_type[k] for k in sorted(by_type)) + note


def _holding_days(buy_date: str, sell_date: str) -> int:
    """Calendar days held. Feeds the fund tiered-redemption table in fee_book.

    Exchange-traded assets ignore the result (A-shares have no short-holding
    penalty), so a parse failure degrading to 0 is safe: it only ever makes a
    fund look MORE expensive, never less.
    """
    try:
        return (datetime.strptime(sell_date, '%Y-%m-%d')
                - datetime.strptime(buy_date, '%Y-%m-%d')).days
    except (ValueError, TypeError) as e:
        logger.debug('[Sim] holding-day parse failed (%s -> %s): %s',
                     buy_date, sell_date, e)
        return 0
...
def _ensure_price_data(
    ...
    """
    from tofu_trading.trading.historical_data import fetch_and_store_price_history, get_price_at
...
def _load_strategy_toolbox(db, uid: int) -> tuple[str, list[dict]]:
    ...
    try:
        rows = db.execute(
            ...
            "FROM trading_strategies WHERE status='active' AND user_id=? "
            "ORDER BY type, name", (uid,)
        ).fetchall()
    except Exception as e:
...
def _load_strategy_analytics(db, uid: int) -> dict[str, Any]:
    ...
    try:
        rows = db.execute(
            ...
            "FROM trading_strategies WHERE user_id=? ORDER BY type, name",
            (uid,)
        ).fetchall()
    except Exception as e:
...
class SimulatorConfig:
    ...
    def __init__(self, **kwargs):
        ...
        # Adaptive stop: the threshold is stated in SIGMA of the holding period,
        # not as a flat percentage. A flat 5% is 1.97 sigma on a broad ETF but
        # only 0.79 sigma on a growth name — i.e. the same number means "rare"
        # for one asset and "routine noise" for another. stop_loss_pct above is
        # kept as the fallback for when history is too short to measure sigma.
        self.adaptive_stop: bool = kwargs.get('adaptive_stop', True)
        self.stop_loss_sigma: float = kwargs.get('stop_loss_sigma', 2.0)
        self.min_stop_pct: float = kwargs.get('min_stop_pct', 3.0)
        self.max_stop_pct: float = kwargs.get('max_stop_pct', 25.0)
        # Fees resolve through the shared FeeBook: per asset type, per trade
        # size, identical to the backtest engine. There is deliberately no
        # buy_fee_rate / sell_fee_rate here — a scalar cannot express the ¥5
        # commission floor, the sell-side-only stamp tax, or the fund tier table.
        self.fee_book = kwargs.get('fee_book') or FeeBook(
            zero_cost=bool(kwargs.get('zero_cost')))
    ...
    def to_dict(self) -> dict:
        # fee_book is a live object, not config data: persist its provenance
        # instead so a stored run records WHICH fee schedule produced its
        # numbers (and whether any symbol was only an estimate).
        d = {k: v for k, v in self.__dict__.items()
             if not k.startswith('_') and k != 'fee_book'}
        d['fee_provenance'] = self.fee_book.provenance()
        d['fee_estimated_symbols'] = self.fee_book.estimated_symbols()
        return d
...
def run_simulation(
    ...
    *,
    ...
    """
    from tofu_trading.trading.historical_data import (
        _ensure_sim_tables,
        build_market_snapshot,
        get_price_at,
    )
    from tofu_trading.trading.intel_timeline import build_intel_context_at
    ...
    # ── Create session ──
    # Minted via the shared helper: the old second-resolution format collided on
    # any two runs started in the same second (double-click, batch sweep, two
    # tenants), and session_id is UNIQUE — the second INSERT raised
    # IntegrityError straight out of this function with no session row.
    session_id = mint_run_id('sim', uid=uid)
    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ...
        try:
            ...
            for sym, pos in list(positions.items()):
                ...
                if price_data:
                    ...
                    # Stop threshold scaled to the volatility of the holding
                    # period actually elapsed (see _adaptive_stop_pct): a flat
                    # percentage means "rare" on a quiet asset and "routine
                    # noise" on a volatile one.
                    _bars_held = max(step_idx - pos.get('buy_step', step_idx), 1)
                    _stop_pct, _stop_basis = _adaptive_stop_pct(
                        db, sym, sim_date, _bars_held, config)
                    if pnl_pct <= -_stop_pct:
                        # Stop-loss. The BREACH is detected on T's close, but the
                        # exit fills at T+1's open — same rule as a discretionary
                        # decision. Filling at the close that triggered the stop
                        # would assume an order placed after the fact.
                        _fill, _fill_err = _next_bar_fill(db, sym, sim_date, config.end_date)
                        if _fill is None:
                            logger.info('[Sim] Stop-loss on %s deferred at %s: %s',
                                        sym, sim_date, _fill_err)
                            continue
                        pos_value = _fill * pos['shares']
                        pnl_pct = (_fill - pos['buy_price']) / pos['buy_price'] * 100
                        current_nav = _fill
                        _hd = _holding_days(pos['buy_date'], sim_date)
                        fee = config.fee_book.fee_for(
                            sym, pos_value, 'sell', holding_days=_hd)['fee_amount']
                        proceeds = pos_value - fee
                        cash += proceeds
                        total_fees += fee
                        total_trades += 1
                        reason = (f'⛔ 止损: {sym} 跌幅 {pnl_pct:.2f}% 超过止损线 '
                                  f'-{_stop_pct:.2f}%（{_stop_basis}，'
                                  f'{config.stop_loss_sigma:g}σ×{_bars_held}栏）')
                        ...
                        trade_log.append({
                            ...
                            'stop_pct': round(_stop_pct, 3), 'stop_basis': _stop_basis,
                    ...
                    if pnl_pct >= abs(config.take_profit_pct):
                        # Take-profit. Fills at T+1's open, same rule as the stop.
                        _fill, _fill_err = _next_bar_fill(db, sym, sim_date, config.end_date)
                        if _fill is None:
                            logger.info('[Sim] Take-profit on %s deferred at %s: %s',
                                        sym, sim_date, _fill_err)
                            continue
                        pos_value = _fill * pos['shares']
                        pnl_pct = (_fill - pos['buy_price']) / pos['buy_price'] * 100
            ...
            strategy_toolbox_prompt, strategy_list = _load_strategy_toolbox(db, uid)
...
### 交易成本（账本实扣，与下单同一来源）
{_describe_fees(config.fee_book, _all_tracked_symbols or config.symbols, min(portfolio_value * config.max_position_pct / 100, max(cash, 1000)))}
...
如果决定观望，输出 <decisions>[]</decisions> 和 <strategies_used>["策略名称"]</strategies_used> 并说明理由。"""
            ...
            for dec in decisions:
                ...
                if action == 'buy' and symbol:
                    ...
                    if amount <= 0:
                        ...
                        continue
                    # Position cap follows CURRENT equity, not initial capital.
                    # Anchored to initial_capital, a portfolio down 50% kept the
                    # same absolute cap — i.e. it silently doubled leverage per
                    # trade exactly when it could least afford it.
                    ...
                    # Execution price: T+1 OPEN, never T's close (see
                    # _next_bar_fill — deciding on T's close and filling at that
                    # same close is a look-ahead no participant can realise).
                    nav, _fill_err = _next_bar_fill(db, symbol, sim_date, config.end_date)
                    if nav is None:
                        logger.info('[Sim] Skip buy %s decided %s: %s', symbol, sim_date, _fill_err)
                        _add_journal(db, session_id, sim_date, 'skip', 'buy', symbol,
                                     _fill_err, dec, confidence)
                        continue

                    fee = config.fee_book.fee_for(symbol, amount, 'buy')['fee_amount']
                    ...
                    # Same rule as the buy side: a decision made on T fills at
                    # T+1's open. No fallback to T's close.
                    close_price, _fill_err = _next_bar_fill(
                        db, symbol, sim_date, config.end_date)
                    if close_price is None:
                        logger.info('[Sim] Skip sell %s decided %s: %s', symbol, sim_date, _fill_err)
                        _add_journal(db, session_id, sim_date, 'skip', 'sell', symbol,
                                     _fill_err, dec, confidence)
                        continue
                    pos_value = close_price * pos['shares']
                    fee = config.fee_book.fee_for(
                        symbol, pos_value, 'sell',
                        holding_days=_holding_days(pos['buy_date'], sim_date))['fee_amount']
                    cash += pos_value - fee
                    total_fees += fee
                    total_trades += 1
    ...
    for sym, pos in list(positions.items()):
        ...
        fee = config.fee_book.fee_for(
            sym, pos_value, 'sell',
            holding_days=_holding_days(pos['buy_date'], config.end_date))['fee_amount']
        cash += pos_value - fee
        total_fees += fee
        total_trades += 1
        pnl_pct = (close_price - pos['buy_price']) / pos['buy_price'] * 100
    ...
    # The equity curve must END at the liquidated portfolio. daily_values is
    # appended at the TOP of each step, i.e. BEFORE that step's trades, so
    # without this the final step's decisions and the forced liquidation were
    # both missing from it — total_return_pct lagged a full step behind, and
    # metrics['final_value'] (curve tail, pre-liquidation) disagreed with the
    # total_pnl written to the DB (cash, post-liquidation). One source now.
    daily_values.append({
    ...
    metrics = _compute_metrics(daily_values, config.initial_capital, total_fees,
                               total_trades, winning_trades, trade_log)
...
def _build_signal_context(db: Any, symbols: list[str], as_of: str) -> str:
    ...
    from tofu_trading.trading.historical_data import get_prices_range
...
def _compute_metrics(
    ...
    ``trade_log`` is optional so existing callers keep working; when supplied it
    yields ``exit_triggers`` — how many exits were stop-losses, take-profits or
    discretionary. Without that breakdown there is no way to tell a run that was
    stopped out repeatedly by noise from one that simply traded, which is
    exactly the question the adaptive stop exists to answer.
    """
    if not daily_values:
        return {}
    ...
    # Exit-trigger breakdown — the diagnostic for "was this stopped out by noise
    # or did it actually trade?". A discretionary exit carries no trigger.
    _exit_triggers: dict[str, int] = {}
    for _t in (trade_log or []):
        if _t.get('action') != 'sell':
            continue
        _key = _t.get('trigger') or 'discretionary'
        _exit_triggers[_key] = _exit_triggers.get(_key, 0) + 1

    # Annualised return, from the REAL calendar span.
    #
    # This used to read ((final/initial) ** (252 / len(daily_values))) — treating
    # the number of SAMPLES as a number of TRADING DAYS. With the default
    # step_days=5 that inflated everything by ~4-6x in both directions: a true
    # 1-year +10% was reported as +61.67%, and a true -10% as -41.2%.
    ann_return = 0.0
    try:
        span_days = (datetime.strptime(dates[-1], '%Y-%m-%d')
                     - datetime.strptime(dates[0], '%Y-%m-%d')).days
    except (ValueError, TypeError) as _e:
        logger.warning('[Sim] Annualisation date parse failed (%s..%s): %s — '
                       'reporting total return unannualised', dates[0], dates[-1], _e)
        span_days = 0
    if span_days >= 1 and initial_capital > 0 and final_value > 0:
        ann_return = ((final_value / initial_capital) ** (365.0 / span_days) - 1) * 100
    elif initial_capital > 0:
        # Sub-one-day span: annualising is meaningless, so report the raw return
        # rather than an explosive extrapolation.
    ...
    return {
        ...
        'decision_points': len(daily_values),
        'exit_triggers': _exit_triggers,
        'stop_loss_exits': _exit_triggers.get('stop_loss', 0),
    }
...
def _compute_benchmark(
    ...
    from tofu_trading.trading.historical_data import get_index_at
