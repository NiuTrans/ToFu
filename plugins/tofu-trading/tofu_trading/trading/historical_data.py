...
def backfill_historical_intel(
    ...
    from tofu_trading.trading.intel import INTEL_SOURCES, crawl_intel_source

    if search_fn is None:
        # Default search function using our web search infrastructure
        def search_fn(query, max_results=8):
            try:
                from tofu_search import perform_web_search as web_search
    ...
    try:
        from tofu_trading.trading.news_apis import fetch_structured_news_sources
...
def _ensure_sim_tables(db: Any):
    ...
    from tofu_trading.storage import _column_exists as _col_exists
