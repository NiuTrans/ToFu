...
can keep doing ``from tofu_trading.trading import get_latest_price`` etc. without
...
from tofu_trading._facade import build_facade
...
from .info import *  # noqa: E402,F401,F403
from .nav import *  # noqa: E402,F401,F403
from .strategy_data import *  # noqa: E402,F401,F403
...
try:
    ...
    _logger.warning('tofu_trading.trading.sources failed to load — multi-source fetchers disabled: %s', _exc, exc_info=True)
...
try:
    ...
    _logger.warning('tofu_trading.trading.intel failed to load — intelligence crawling disabled: %s', _exc, exc_info=True)
...
try:
    ...
    _logger.warning('tofu_trading.trading.backtest failed to load — backtesting disabled: %s', _exc, exc_info=True)
...
try:
    ...
    _logger.warning('tofu_trading.trading.market failed to load — market data disabled: %s', _exc, exc_info=True)
...
try:
    ...
    _logger.warning('tofu_trading.trading.screening failed to load — ETF/stock screening disabled: %s', _exc, exc_info=True)
...
try:
    ...
    _logger.warning('tofu_trading.trading.intel_timeline failed to load — time-locked intel disabled: %s', _exc, exc_info=True)
