...
def _nav_from_holdings(code):
    ...
    try:
        ...
        db = get_thread_db(
            DOMAIN_TRADING, owner_user_id=DEFAULT_OWNER_ID)
        # L3 price fallback: uses the default owner's cost basis as a
        # last-resort price when every market source is down. A price is not
        # user-owned data, but the row it is read from is -- so it is scoped.
        row = db.execute(
            'SELECT symbol, asset_name, buy_price, buy_date FROM trading_holdings '
            'WHERE symbol=? AND user_id=? LIMIT 1',
            (code, DEFAULT_OWNER_ID)
        ).fetchone()
        if row:
            d = dict(row)
            return {
