...
def seed_builtin_strategies(db, uid=None):
    """Insert built-in strategies for one user if they don't exist yet.

    ``uid`` defaults to the default owner: seeding runs at startup, outside any
    request, and the built-in set is identical for everyone.
    """
    if uid is None:
        from tofu_trading.identity import DEFAULT_OWNER_ID
        uid = DEFAULT_OWNER_ID
    existing = db.execute(
        'SELECT name FROM trading_strategies WHERE source=? AND user_id=?',
        ('builtin', uid)).fetchall()
    ...
    for s in BUILTIN_STRATEGIES:
        if s['name'] in existing_names:
            continue
        db.execute(
            '''INSERT INTO trading_strategies (user_id, name, type, status, logic, scenario, assets, result, source, created_at, updated_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?)''',
            (uid, s['name'], s['type'], 'active', s['logic'],
...
def seed_builtin_strategy_groups(db, uid=None):
    ...
    strategies = db.execute(
        "SELECT id, name, type FROM trading_strategies "
        "WHERE source='builtin' AND status='active' AND user_id=?", (uid,)).fetchall()
    if not strategies:
        return 0
...
def record_decision(db, batch_id, strategy_group_id, strategy_group_name,
    ...
    db.execute(
        '''INSERT INTO trading_decision_history
           (user_id, batch_id, strategy_group_id, strategy_group_name, briefing_content,
            recommendation_content, trades_json, status, created_at)
           VALUES (?,?,?,?,?,?,?,?,?)''',
        (uid, batch_id, strategy_group_id, strategy_group_name or '',
