...
def record_crawl(db, crawl_date, category, source_key, items_fetched, status='ok'):
    ...
    from tofu_trading.storage import db_execute_with_retry
...
def compute_content_fingerprint(title, snippet=''):
    ...
    """
    from tofu_trading.trading.simhash import compute_simhash
...
def deduplicate_intel(db, title, source_url, category, snippet=''):
    ...
    """
    from tofu_trading.trading.simhash import hamming_distance
    ...
    if candidate_hash != 0:
        ...
        for row in recent_hashes:
            ...
            from tofu_trading.trading.simhash import to_unsigned64
...
def _purge_expired_snapshot(db, category):
    ...
    try:
        ...
        with write_transaction(db, label='purge expired intel snapshot'):
            cur = db.execute(
                'DELETE FROM trading_intel_cache '
                'WHERE category=? AND expires_at < ? AND expires_at != ?',
                (category, now_str, '')
            )
...
def cleanup_stale_intel(db):
    ...
    from tofu_trading.transactions import write_transaction
    try:
        # DELETE starts a SQLite write transaction even when rowcount is zero.
        # Own one explicit boundary so the no-op housekeeping case cannot pin
        # the process-wide writer lane until the next restart.
        with write_transaction(db, label='cleanup stale intel'):
            for cat, src in INTEL_SOURCES.items():
                window = src.get('decision_window_days', 7)
                max_days = int(window * 1.5)
                max_days = max(max_days, 7)
                cutoff = (now - timedelta(days=max_days)).strftime('%Y-%m-%d %H:%M:%S')
                cur = db.execute(
                    'DELETE FROM trading_intel_cache '
                    'WHERE category=? AND fetched_at < ? AND fetched_at != ?',
                    (cat, cutoff, '')
                )
                deleted = cur.rowcount
                if deleted:
                    total_deleted += deleted
                    logger.info('🧹 Housekeeping: removed %d old [%s] records (>%dd)',
                                deleted, cat, max_days)
    except Exception as e:
        logger.error('Housekeeping failed atomically: %s', e, exc_info=True)
        return 0
    if total_deleted:
...
def _reconnect_if_dead(db, exc):
    ...
    if is_dead:
        ...
        return get_thread_db(
            DOMAIN_TRADING,
            owner_user_id=getattr(db, "owner_user_id", DEFAULT_OWNER_ID),
        )
    return db
...
def crawl_intel_source(db, category, query, search_fn, analyze_fn=None, crawl_date=None,
    ...
    try:
        ...
        if use_multi_source:
            try:
                from tofu_trading.trading.sources import multi_source_search
        ...
        for r in results:
            ...
            from tofu_trading.trading.sources import _is_blocked_url
            ...
            from tofu_trading.trading.simhash import to_signed64
            content_hash = to_signed64(content_hash)

            # Never carry a write transaction into the next page fetch/LLM
            # call. One previous crawl held SQLite's global writer lane for
            # 25+ minutes because INSERT happened inside this loop while the
            # only commit sat after every remaining network operation.
            from tofu_trading.transactions import write_transaction
            with write_transaction(db, label='insert crawled intel item'):
                db.execute(
