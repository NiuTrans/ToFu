...
# Tools listed here are DEFERRED (not in the initial tool list).
# They become available when the model calls tool_search.
DEFERRED_TOOL_HINTS: dict[str, str] = {
    # Browser tools — only useful when browser is connected
    'browser_navigate':     'browser navigate open url page web',
    'browser_click':        'browser click element button link tap',
    'browser_type':         'browser type input text field form keyboard',
    'browser_screenshot':   'browser screenshot capture page visual',
    'browser_read_tab':     'browser read tab content text page DOM',
    'browser_get_interactive_elements': 'browser interactive elements buttons links inputs forms',
    'browser_execute_js':   'browser execute javascript run script console eval',
    ...
    # Desktop agent tools (if available)
...
def partition_tools(tool_list: list,
    ...
    # Phase 1: Static deferral (tools explicitly in DEFERRED_TOOL_HINTS)
    for tool_def in tool_list:
        fn_name = tool_def.get('function', {}).get('name', '')
        if fn_name in DEFERRED_TOOL_HINTS and fn_name not in CORE_TOOL_NAMES:
