...
Houses ``run_task_sync`` — the entry point for the Feishu bot / scheduled
tasks / API consumers that just need the final answer text — plus its
progress helpers ``_format_progress_event`` and ``_drain_progress`` (both
exercised directly by ``tests/test_run_task_sync_progress.py``).

Dependency direction: leaf module — imports only ``agent_core.events`` and
``tasks_pkg.manager`` / ``tasks_pkg.orchestrator``.

This owner is independent of orchestration and chat-flow execution.
"""
...
def run_task_sync(
    ...
    *,
    ...
    task = create_task(conv_id, messages, cfg, user_id=int(user_id))
    ...
    def _worker():
        try:
            ...
            task['error'] = _err_from_exc(
                ...
                context='run_task_sync', source='sync-run',
            )
            task['status'] = 'error'
        finally:
