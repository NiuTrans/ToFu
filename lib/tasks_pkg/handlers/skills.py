"""lib/tasks_pkg/handlers/skills.py — Skills tool handler (load_skill).

The skills channel is READ-ONLY for the model: it can load a
...
def _handle_skill_tool(task, tc, fn_name, tc_id, fn_args, rn, round_entry,
    ...
    """Handle load_skill — load a skill's full guide + file manifest.
    ...
    # under any attached root can be loaded.
    _extra_paths = []
    ...
    try:
        tool_content = load_skill(
            fn_args.get('skill_id', ''),
            project_path=_proj,
            extra_paths=_extra_paths,
        )
        ok = tool_content.startswith('Skill loaded:')
        meta = _build_simple_meta(
            fn_name, tool_content,
            source='Skill',
            title=fn_args.get('skill_id', ''),
            snippet=tool_content.split('\n', 1)[0][:120],
            badge='📦 loaded' if ok else '❌ not loaded',
        )
    except Exception as e:
        logger.error('[Skills] load_skill failed: %s', e, exc_info=True)
        tool_content = f'Failed to load skill: {str(e)}'
        meta = _build_simple_meta(
            fn_name, tool_content,
            source='Skill',
            title=fn_args.get('skill_id', ''),
