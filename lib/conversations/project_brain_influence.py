...
produced from the same providers the Context Composer calls —
...
module previously reported as healthy:
  1. The GOALS lane did not exist here at all, while the composer was
...
def build_conv_influence(project_path: str, conv_id: str, *, user_id: int) -> dict:
    ...
    try:
        ...
        from lib.conversations.project_charter import _INJECTION_DECISION_WINDOW
        ...
        for d in (rec.get("decisions") or [])[-_INJECTION_DECISION_WINDOW:]:
            if isinstance(d, dict):
                ...
                decisions.append(
                    {"text": str(d), "summary": "", "kind": "", "ts": 0, "by_conv": ""}
                )
        decisions.reverse()
        ...
        out["charter"]["contentSet"] = bool((rec.get("content") or "").strip())
    ...
    try:
        ...
        out["goals"]["items"] = [
            {"text": it.get("text") or ""}
            for it in (
                list_watch_items(project_path, user_id=user_id).get("items") or []
            )
            if it.get("kind") == "goal"
        ...
        logger.debug(
            "[BrainInfluence] goals read failed proj=%.40r: %s", project_path, e
        )
    ...
    try:
        ...
        logger.debug(
            "[BrainInfluence] board read failed proj=%.40r: %s", project_path, e
        )
    ...
    try:
        ...
        pending = pending_proposals(project_path, user_id=user_id)
        ...
        logger.debug(
            "[BrainInfluence] pending read failed proj=%.40r: %s", project_path, e
        )
