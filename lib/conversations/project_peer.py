...
def _prune_and_check(
    ...
    *,
    ...
    max_n: int = _PEER_MSG_MAX_PER_WINDOW,
) -> tuple:
...
def _refund_rate_slot(
    ...
    *,
    ...
    """
    key = (int(user_id), from_conv, to_conv)
    with _rate_lock:
...
def _join_peers(
    peers: list, task_by_conv: dict, claim_by_conv: dict, *, exclude_conv: str = ""
) -> list:
...
def _live_task_by_conv(*, user_id: int) -> dict:
...
def _titles_by_conv(conv_ids, *, user_id: int) -> dict:
    ...
    ``conv <id>``. This backfills the stored title from the conversation
    authority. When a conversation has no usable title (never titled), it falls
    ...
    Best-effort: returns ``{}`` on any read failure so the caller degrades to the
    ...
    try:
        ...
        for row in rows:
            ...
            if title and title.lower() != "untitled":
                ...
                snippet = first_user_text(row.messages, max_chars=60)
                if snippet:
                    out[row["id"]] = snippet
    except Exception as e:
...
def build_peer_status(project_path: str, conv_id: str = "", *, user_id: int) -> dict:
    ...
    try:
        ...
        logger.debug(
            "[PeerStatus] presence read failed proj=%.40r: %s", project_path, e
        )
    ...
    try:
        ...
        claim_by_conv = claims_by_conv(
            read_board(project_path, user_id=user_id).get("tasks", [])
        )
    except Exception as e:
        logger.debug("[PeerStatus] board join failed proj=%.40r: %s", project_path, e)

    task_by_conv = _live_task_by_conv(user_id=user_id)
    ...
    if need_titles:
        titles = _titles_by_conv(need_titles, user_id=user_id)
        for p in view:
            if not p.get("title"):
    ...
    out["convCount"] = len(
        {p.get("convId") for p in view if p.get("convId") and not p.get("agentId")}
    )
    return out
...
def _resolve_target_conv_id(to_conv_id: str, *, user_id: int) -> tuple:
    ...
    try:
        ...
        exact = get_conversation(tid, user_id=int(user_id), include_messages=False)
        if exact:
            return exact["id"], ""
        rows = list_conversations(
            user_id=int(user_id), limit=10_000, include_messages=False, settings_keys=[]
        )
        matches = [row["id"] for row in rows if row["id"].startswith(tid)]
        if len(matches) == 1:
            return matches[0], ""
        if len(matches) > 1:
            logger.info(
                "[PeerMsg] target id/prefix %s is AMBIGUOUS (%d matches)",
                tid[:12],
                len(matches),
            )
            return "", "ambiguous_target"
        logger.info("[PeerMsg] target id/prefix %s matches no conversation", tid[:12])
        return "", "unknown_target"
    except Exception as e:
        logger.warning("[PeerMsg] target-id resolve failed for %s: %s", tid[:12], e)
...
def _live_drain_eligible_task(conv_id: str, *, user_id: int) -> str:
    ...
    try:
        ...
        for t in chat_task_runtime.snapshot_owned(user_id=int(user_id)):
                ...
                if t.get("convId") != conv_id and t.get("_peer_drain_key") != conv_id:
                    continue
        ...
        logger.debug(
            "[PeerMsg] live-task eligibility probe failed for conv=%s: %s",
            conv_id[:8],
            e,
        )
    return ""
...
def send_peer_message(
    ...
    *,
    ...
    Args:
        ...
        wake: Codex ``trigger_turn`` semantics (codex-rs agent_communication:
            send_message = mailbox-only vs followup_task = start a turn NOW).
            True (default): an IDLE target is dispatched a fresh turn to answer
            the note (send-time drain + the 30 s heartbeat idle-drain). False:
            mailbox-only — the row is flagged ``_peerNoWake`` and NEITHER idle
            drain spins up a turn for it; a LIVE target still receives the
            fast-path inbox twin (delivered into the EXISTING turn at its next
            round boundary — no new turn), and an idle target sees the note on
            its next NATURAL turn (completion dispatch, startup redispatch, or
            a human turn). Exactly-once delivery is unchanged — only the
            wake-up is suppressed. Use wake=False for FYI notes that must not
            burn a whole LLM turn on an idle sibling.
    ...
    to_conv_id, _resolve_err = _resolve_target_conv_id(to_conv_id, user_id=user_id)
    if _resolve_err:
        logger.info(
            "[PeerMsg] refused %s→%s: %s",
            from_conv_id[:8],
            (to_conv_id or "?")[:8],
            _resolve_err,
        )
    ...
    now = time.time()
    allowed, retry_after = _rate_gate(
        from_conv_id, to_conv_id, now, user_id=user_id)
    if not allowed:
        logger.info(
            "[PeerMsg] rate-limited %s→%s (retry in %.0fs)",
            from_conv_id[:8],
            to_conv_id[:8],
            retry_after,
        )
    ...
    if human:
        body = (
            f"[Message from the project operator, relayed via a sibling "
        ...
        body = (
            f"[Peer message from a sibling conversation of this project "
    ...
    payload = {
        "text": body,
        # Queue previews render the sender's unframed note while the model
        # receives ``text`` with its provenance and safety framing.
        "_peerText": text,
        "_peerMessage": True,
        "_fromConv": from_conv_id,
    }
    if human:
        payload["_peerHuman"] = True
    if not wake:
        payload["_peerNoWake"] = True
    try:
        from lib.message_queue import KIND_PEER_MSG, enqueue_message

        res = enqueue_message(
            to_conv_id, payload, config or {}, kind=KIND_PEER_MSG, user_id=user_id
        )
    except Exception as e:
        ...
        _refund_rate_slot(
            from_conv_id, to_conv_id, now, user_id=user_id)
        logger.error(
            "[PeerMsg] enqueue failed %s→%s (rate slot refunded): %s",
            from_conv_id[:8],
            to_conv_id[:8],
            e,
            exc_info=True,
    ...
    if queue_id and _live_drain_eligible_task(to_conv_id, user_id=user_id):
        ...
        try:
            ...
            logger.info(
                "[PeerMsg] fast-path inbox twin queued for live conv %s (queueId=%s)",
                to_conv_id[:8],
                (queue_id or "?")[:8],
            )
        except Exception as e:
            logger.warning(
                "[PeerMsg] fast-path inbox twin failed for conv=%s "
    ...
    logger.info(
        "[PeerMsg] %s→%s queued=%s (%d chars)",
        from_conv_id[:8],
        to_conv_id[:8],
        (res.get("queueId") or "?")[:8],
        len(text),
    )
    ...
    # wake=False (mailbox-only): skip the send-time idle drain — the whole
    # point is NOT to spin up a fresh turn on an idle target. The heartbeat
    # idle-drain likewise skips ``_peerNoWake`` rows (message_queue).
    if queue_id and wake:
        try:
            if not _live_drain_eligible_task(to_conv_id, user_id=user_id):
                ...
                if _tid:
                    logger.info(
                        "[PeerMsg] idle target %s drained at send time "
            ...
            logger.warning(
                "[PeerMsg] send-time idle drain failed conv=%s "
...
def intervene_peer(
    ...
    *,
    ...
    if hard_abort and not (approved_by or "").strip() and approval_fn is not None:
        prompt = (
            f"Conversation {from_conv_id[:8]} requests a HARD ABORT of "
        ...
        try:
            ...
            logger.warning(
                "[Intervene] approval_fn raised %s→%s: %s",
                from_conv_id[:8],
                to_conv_id[:8],
                e,
            )
        if approver:
            ...
            logger.info(
                "[Intervene] hard-abort DENIED by human %s→%s",
                from_conv_id[:8],
                to_conv_id[:8],
            )
    ...
    if not allowed:
        logger.info(
            "[Intervene] hard-abort refused %s→%s: %s",
            from_conv_id[:8],
            to_conv_id[:8],
            reason,
        )
    ...
    if not hard_abort:
        ...
        note = message or (
            "Heads-up: a sibling conversation believes your current "
        ...
        res = send_peer_message(
            project_path,
            from_conv_id,
            to_conv_id,
            note,
            config=config,
            _kind_label="intervention",
            user_id=user_id,
        )
        res["mode"] = "advisory"
        return res
    ...
    try:
        ...
        n = abort_running_tasks_for_conv(to_conv_id, user_id=user_id)
    except Exception as e:
        logger.error(
            "[Intervene] abort failed %s→%s: %s",
            from_conv_id[:8],
            to_conv_id[:8],
            e,
            exc_info=True,
    ...
    logger.info(
        "[Intervene] HARD ABORT %s→%s aborted=%d approved_by=%s",
        from_conv_id[:8],
        to_conv_id[:8],
        n,
        approved_by,
...
def _fmt_feed(events: list, current_conv_id: str = "") -> str:
    ...
    if not events:
        return (
            "No recent cross-conversation activity in this project. (The "
...
def _fmt_peer_status(status: dict, current_conv_id: str) -> str:
    ...
    if not peers:
        return (
            "No other conversations of this project are active right now. "
...
def execute_peer_tool(
    ...
    *,
    ...
    try:
        if not project_path:
            return (
                "Error: peer collaboration tools are only available in "
                "project mode (open a project first)."
            )
        if fn_name == "project_peer_status":
            target = (fn_args.get("conv_id") or "").strip()
            status = build_peer_status(project_path, current_conv_id, user_id=user_id)
            if target:
                status["peers"] = [
                    p
        ...
        if fn_name == "project_feed_read":
            ...
            feed = read_project_feed(
                project_path, user_id=user_id, limit=limit)
        ...
        if fn_name == "project_message":
            ...
            res = send_peer_message(
                ...
                text,
                config=config,
                wake=True if wake is None else bool(wake),
                user_id=user_id,
            )
            if res.get("ok"):
                if wake is False:
                    return (
                        f"Message filed to conversation {to_conv[:8]}'s "
                        f"mailbox (wake=false) — NO new turn was started; "
                        f"the peer sees it on its next natural turn. This "
                        f"is advisory; the peer decides whether to act on it."
                    )
                return (
                    f"Message delivered to conversation {to_conv[:8]} — it "
            ...
            if res.get("error") == "rate_limited":
                return (
                    f"Not sent — you have reached the peer-message rate limit "
        ...
        if fn_name == "project_intervene":
            ...
            approved_by = str(
                (config or {}).get("interventionApprovedBy") or ""
            ).strip()
            res = intervene_peer(
                project_path,
                current_conv_id,
            ...
            if res.get("ok"):
                if res.get("mode") == "hard_abort":
                    return (
                        f"Hard intervention executed (human-approved) — aborted "
                ...
                return (
                    f"Advisory intervention sent to conversation {to_conv[:8]} "
            ...
            if res.get("error") == "denied_by_human":
                return (
                    f"Hard abort of conversation {to_conv[:8]} was DENIED by "
            ...
            if res.get("error") == "hard_abort_requires_approval":
                return (
                    "A hard abort of another conversation's running task "
            ...
            if res.get("error") == "rate_limited":
                return (
                    f"Not sent — intervention rate limit reached for "
...
__all__ = [
    ...
    "_join_peers",
    "_prune_and_check",
    "_authorize_hard_abort",
    "_fmt_feed",
    "_PEER_MSG_MAX_PER_WINDOW",
    "_PEER_MSG_WINDOW_S",
]
