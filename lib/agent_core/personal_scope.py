...
__all__ = [
    ...
    'resolve_paper_termfill_enabled',
]
...
PERSONAL_CAPABILITIES: dict[str, PersonalCapability] = {
    ...
    'paperTermfillEnabled': PersonalCapability(
        ...
        ui_default=True,
        summary=(
            'The Paper Reading-Mode terminology definition-backfill second '
            'pass: when the report glossary has genuine gaps (a term used in '
            'the body with no definition available to the reader), a bounded '
            'pure-body-context LLM call generates the missing definitions and '
            'appends a re-audit-gated glossary addendum, so the report becomes '
            "self-contained. Injects no operator state, but — like "
            'langCorrectionEnabled — it SILENTLY BILLS an LLM call, so it must '
            'fail closed on headless / BYO surfaces unless the caller opts in. '
            'ON by default for the interactive reader (measured: ~75% gap '
            'closure, 4.9/5 correctness). Its own engine gates it, so '
            'prompt_block is empty.'),
        prompt_block=''),
}
...
def resolve_paper_termfill_enabled(cfg: dict | None) -> bool:
    """May the terminology definition-backfill pass run for this report?

    Resolution mirrors ``resolve_paper_insight_personal_context``:

      * Explicit ``paperTermfillEnabled`` in cfg (set by a headless builder via
        the registry → fail-closed False, or an opt-in caller) is honoured.
      * Absent (the interactive UI report route passes no such key) → default
        TRUE, so the interactive reader gets self-contained reports by default.

    Every headless cfg-builder runs ``apply_headless_personal_defaults`` which
    stamps this key False, so a headless / BYO caller lands on ``False`` unless
    they opted in. This is the per-request gate; a fleet-wide kill switch lives
    separately in ``terminology_backfill.termfill_globally_disabled``.
    """
    if isinstance(cfg, dict) and 'paperTermfillEnabled' in cfg:
        return bool(cfg['paperTermfillEnabled'])
    return True
