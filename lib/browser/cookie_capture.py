...
_last_attempt: dict[str, float] = {}                # domain → last login-tab open
...
def _background_capture(dom: str, url: str) -> None:
    ...
    try:
        ...
        audit_log('cookie_capture_login_tab', domain=dom, url=url[:200])
...
def handle_login_wall(url: str, final_url: str = '') -> bool:
    ...
    with _capture_lock:
        if dom in _capture_threads:
            ...
            return False
        if time.time() - _last_attempt.get(dom, 0.0) < _ATTEMPT_COOLDOWN_S:
