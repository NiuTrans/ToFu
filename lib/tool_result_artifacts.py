...
def continuation_display_label(fn_name, fn_args, provenance):
    ...
    row) and WHICH call's spilled result is being read.
    """
    source = str((provenance or {}).get('display') or '').strip()
    if not source:
        source = str((provenance or {}).get('toolName') or '').strip()
    if len(source) > 96:
        source = source[:95].rstrip() + '…'
    llm_round = (provenance or {}).get('llmRound')
    label = ('Read compacted result' if fn_name == 'read_tool_artifact'
             else 'Search compacted result')
    if isinstance(llm_round, int):
        label += f' of R{llm_round + 1}'
    if source:
        label += f' · {source}'
    if fn_name == 'search_tool_artifact':
        query = ' '.join(str((fn_args or {}).get('query') or '').split())
        if query:
            label += f': {query}'
    return label
