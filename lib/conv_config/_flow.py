...
pair that ``lib.orchestration_chat_flow_runner.resolve_chat_flow_entry`` reads.
Unknown builtins (for example ``builtin:unknown``) tombstone to no
selection so a legacy conversation degrades to a normal chat.
"""
...
def _parse_active_flow(value: Any) -> tuple[str, str]:
    ...
    ``lib.orchestration_chat_flow_runner.resolve_chat_flow_entry`` reads.
    """
    if not isinstance(value, str) or not value:
        return '', ''
    if value.startswith('builtin:'):
        name = value[len('builtin:'):]
        return (name, '') if name in CHAT_FLOW_BUILTINS else ('', '')
    return '', value
