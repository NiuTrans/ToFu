...
def test_unknown_conv_flagged(flask_client):
    ...
    assert any(d['convId'] == conv_id and d['kind'] == 'unknown_conv'
               for d in divs), body
    # The lone queried conv is provably a ghost: the client must be told to
    # evict it, or it re-reports the phantom on every probe forever
    # (2026-08-19: 154 unknown_conv rows for one conv deleted on a 2nd tab).
    assert body['deletedConvIds'] == [conv_id], body


def test_unknown_conv_directive_suppressed_on_mass_unknown(flask_client):
    """Safety gate: EVERY queried conv unknown + zero resolved is
    indistinguishable from an authority that just lost its data — the mass
    eviction directive must be suppressed (warnings still fire)."""
    ghost_a, ghost_b = _cid(), _cid()
    resp = _post(flask_client, [{'convId': ghost_a, 'taskIds': [], 'rev': 3},
                                {'convId': ghost_b, 'taskIds': [], 'rev': 4}])
    body = resp.get_json()
    assert sum(1 for d in body['divergences'] if d['kind'] == 'unknown_conv') == 2
    assert body['deletedConvIds'] == [], body


def test_unknown_conv_directive_emitted_when_authority_resolves(flask_client):
    """Mixed batch: the authority demonstrably answered (one conv resolved),
    so the remaining unknown conv is a provable ghost → directive emitted."""
    known, ghost = _cid(), _cid()
    _insert_conv(flask_client, known, rev=7)
    resp = _post(flask_client, [{'convId': known, 'taskIds': [], 'rev': 7},
                                {'convId': ghost, 'taskIds': [], 'rev': 3}])
    body = resp.get_json()
    assert body['deletedConvIds'] == [ghost], body
