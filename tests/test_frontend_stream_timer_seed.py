#!/usr/bin/env python3
"""Frontend: the server-authoritative elapsed-timer seed that keeps the
timer from restarting at 0 on refresh.

`_seedStreamTimerStart(convId, serverStartMs)` (core/health_stream_timer.js)
...
The former `_bumpConvOnOpen` "click an old conv → float to top" half of this
file was removed with the feature itself (owner directive 2026-08-14:
opening/browsing a conversation must not rewrite its `updatedAt` recency).
...
TIMER_JS = Path(runtime_section_path('core/health_stream_timer.js'))
