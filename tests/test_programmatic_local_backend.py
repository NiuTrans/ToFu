...
handler's read-only eligibility gate for PTC-active rounds.
"""
...
from lib.tasks_pkg.tool_orchestration_policy import resolve_tool_orchestration
...
def test_local_guidance_tiers():
    ...
    assert 'program' in prog and 'task-approved read-only tools' in prog
    assert 'read_files' not in prog and 'grep_search' not in prog
    assert 'read-only' in prog
...
def test_ptc_projection_appends_program_tier():
    ...
    assert 'task-approved read-only tools' in execute['function']['description']
...
# ── handler-side read-only eligibility gate ─────────────────────────
...
def _mk_task(ptc_local=None):
    task = {
        ...
        '_executable_tool_catalog': [
            _fn('read_files'), _fn('grep_search'), _fn('todo_write')],
    }
...
def test_program_gate_rejects_non_eligible_tool():
    task = _mk_task({'tier': 'program', 'eligible': ['read_files']})
    payload = _run_program(
        task, "const r = tools.call('todo_write', {'todos': []}); return r;")
    assert payload['status'] == 'error'
    assert 'tool_not_program_eligible' in json.dumps(payload)


def test_program_gate_allows_eligible_tool():
    task = _mk_task({'tier': 'program', 'eligible': ['read_files']})
    payload = _run_program(
        task, "const r = tools.call('read_files', "
              "{'paths': ['__missing__.txt']}); return r;")
    assert 'tool_not_program_eligible' not in json.dumps(payload)


def test_program_gate_inactive_without_ptc_latch():
    task = _mk_task()
    payload = _run_program(
        task, "const r = tools.call('todo_write', {'todos': []}); return r;")
    assert 'tool_not_program_eligible' not in json.dumps(payload)
...
def test_program_gate_allows_search_tools_discovery():
    task = _mk_task({'tier': 'program', 'eligible': ['read_files']})
...
def test_local_guidance_dependent_chain_example():
    ...
    assert 'dependent reads and compact JSON' in prog
    # The batch override shape stays free of program guidance.
    assert 'dependent reads and compact JSON' not in local_ptc_guidance(
        'batch', ['read_files'])
...
def test_oversized_gateway_is_compacted_instead_of_aborting_request():
    ...
    assert _wire_names(out) == ['search_tools', 'execute_tools']
    assert gw.tool_schema_tokens(out, model='kimi-k3') \
        <= gw.LOCAL_GATEWAY_MAX_TOKENS
    assert 'query' in out[0]['function']['parameters']['properties']
    assert {'calls', 'program'} <= set(
        out[1]['function']['parameters']['properties'])
...
def test_budget_compaction_preserves_required_parameter_named_description():
    ...
    out = gw.fit_tool_schema_budget(
        [PROJECT_TOOL_WRITE_FILE], budget_tokens=100, model='kimi-k3')
    ...
    assert gw.tool_schema_tokens(out, model='kimi-k3') <= 100
...
def test_kimi_has_no_implicit_direct_schema_budget():
    from lib.llm._sse_core import prepare_request
    ...
    assert gw.tool_schema_tokens(body['tools'], model='kimi-k3') > 4_000
