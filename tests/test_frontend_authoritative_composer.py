...
_buildConvConfig = async () => ({ autoTranslate: false });
_buildConvSettings = async () => ({});
...
createOptimisticUserTurn = (input) => ({
...
pendingPdfTexts = [sentPdf];
_buildConvConfig = async () => ({ autoTranslate: true });
runtimeScope.ConversationTurnStore.submitConversation = async () => {
...
check(observed.echoCount === 1,
  'optimistic user turn was not painted before the command round trip');
...
check(optimisticTurnRemovals.length === 1
  && optimisticTurnRemovals[0].turnId === observed.echoTurnId,
  'accepted acknowledgement did not remove exactly the optimistic echo');
check(composerInput.value === '' && pendingImages.length === 0,
...
check(optimisticTurnUpserts.length === 1,
  'failed send never painted the optimistic echo');
check(optimisticTurnRemovals.length === 1
  && optimisticTurnRemovals[0].turnId === optimisticTurnUpserts[0].turn.turnId,
  'failed send did not remove exactly the optimistic echo');
...
check(composerInput.value === 'captured before failure\ntyped while sending',
  'failure restore clobbered text typed while the send was in flight');
check(pendingImages.length === 2 && pendingImages[0] === sentImage
  && pendingImages[1] === addedImage,
