...
class TestToolDeferral:
    ...
    def test_partition_deferred_tools_removed(self):
        from lib.tools.deferral import partition_tools
        tools = [
            self._make_tool('read_files'),
            self._make_tool('browser_navigate'),
            self._make_tool('browser_click'),
        ]
        core, deferred = partition_tools(tools)
        core_names = {t['function']['name'] for t in core}
        deferred_names = {t['function']['name'] for t in deferred}
        assert 'read_files' in core_names
        assert 'browser_navigate' in deferred_names
        assert 'browser_click' in deferred_names

    def test_partition_adds_tool_search(self):
        """When there are deferred tools, tool_search is auto-added."""
        from lib.tools.deferral import partition_tools
        tools = [
            self._make_tool('read_files'),
            self._make_tool('browser_navigate'),
    ...
    def test_search_finds_matching_tools(self):
        ...
        assert 'browser_navigate' in matched_names
        assert 'browser_click' in matched_names
    ...
    def test_search_no_match(self):
        from lib.tools.deferral import search_deferred_tools
        deferred = [self._make_tool('browser_navigate')]
    ...
    def test_format_results_with_matches(self):
        ...
        tools = [self._make_tool('browser_navigate')]
    ...
    def test_core_tool_not_deferred_even_if_in_hints(self):
        """A tool in CORE_TOOL_NAMES should never be deferred even if listed in hints."""
        from lib.tools.deferral import CORE_TOOL_NAMES, DEFERRED_TOOL_HINTS, partition_tools
        # Ensure no core tool is in the deferred hints
        overlap = CORE_TOOL_NAMES & set(DEFERRED_TOOL_HINTS.keys())
        assert len(overlap) == 0, f'Core tools found in deferred hints: {overlap}'

    def test_scheduler_tools_deferred(self):
        from lib.tools.deferral import partition_tools
        tools = [
            self._make_tool('read_files'),
        ...
        core, deferred = partition_tools(tools)
        deferred_names = {t['function']['name'] for t in deferred}
        assert 'create_scheduled_task' in deferred_names
        assert 'list_scheduled_tasks' in deferred_names
...
class TestToolSearchHandler:
    ...
    def test_handler_discovers_and_activates_tools(self):
        ...
        import threading
        deferred = [self._make_tool('browser_navigate'), self._make_tool('generate_image')]
...
class TestFullPipelineIntegration:
    ...
    def test_deferral_partition_real_tools(self):
        """Test partitioning with realistic tool definitions."""
        from lib.tools.deferral import partition_tools
        ...
        for name in ['browser_navigate', 'browser_click', 'browser_type',
                      'browser_screenshot', 'generate_image',
        ...
        # All core tools preserved
        assert 'read_files' in core_names
        assert 'web_search' in core_names
        assert 'emit_to_user' in core_names
        # Browser/image/scheduler deferred
        assert 'browser_navigate' in deferred_names
        assert 'generate_image' in deferred_names
        assert 'create_scheduled_task' in deferred_names
        # tool_search added
        assert 'tool_search' in core_names
        # Total unchanged
        assert len(core) + len(deferred) == len(tools) + 1  # +1 for tool_search
