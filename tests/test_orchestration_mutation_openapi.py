...
def test_mutation_payload_schema_derives_versioned_policy_fields():
    import lib.orchestration.mutation_contract as mutation_contract_module
    ...
    assert 'legacyTargetFields' not in contract
    assert 'legacyStatusFields' not in contract
    assert openapi_module.mutation_payload_schema is \
        mutation_contract_module.mutation_payload_schema
    assert 'def mutation_payload_schema' not in open(
        openapi_module.__file__, encoding='utf-8').read()


def test_mutation_response_narrows_action_and_reason_fields():
    import lib.orchestration.mutation_contract as mutation_contract_module
    import routes.api_v1.orchestration_mutation_openapi as openapi_module
    ...
    assert set(schema['required']) == {'ok', 'mutation', 'error'}
    ...
    assert openapi_module.mutation_response_schema is \
        mutation_contract_module.mutation_response_schema
...
def test_mutation_endpoint_contracts_publish_only_canonical_policy():
    assert all('compatibility' not in contract
               for contract in mutation_endpoint_contracts().values())
