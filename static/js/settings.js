...
const _ICON_V = '0.5.1';  // cache-bust version — bump when icons change
...
const _PROVIDER_TEMPLATES = [
  ...
  },
  {
    ...
    models: [
      // ── Qwen Plus (flagship) ──
      { model_id: 'qwen3.6-plus',       capabilities: ['text', 'vision', 'thinking'],  rpm: 60,  cost: 0.002 },  // ¥2/¥12 (≤256K)
      { model_id: 'qwen3.5-plus',       capabilities: ['text', 'vision', 'thinking'],  rpm: 60,  cost: 0.001 },  // ¥0.8/¥4.8 (≤128K)
      // ── Qwen Max ──
      { model_id: 'qwen3-max',          capabilities: ['text', 'thinking'],             rpm: 30,  cost: 0.004 },  // ¥2.5/¥10 (≤32K)
      { model_id: 'qwen-max',           capabilities: ['text'],                         rpm: 30,  cost: 0.003 },  // ¥2.4/¥9.6
      // ── Qwen Plus (legacy) ──
      { model_id: 'qwen-plus',          capabilities: ['text', 'thinking'],             rpm: 60,  cost: 0.002 },  // ¥0.8/¥2 non-think
      // ── Qwen Flash ──
      { model_id: 'qwen3.5-flash',      capabilities: ['text', 'thinking', 'cheap'],    rpm: 120, cost: 0.001 },  // ¥0.2/¥2 (≤128K)
      { model_id: 'qwen-flash',         capabilities: ['text', 'thinking', 'cheap'],    rpm: 120, cost: 0.001 },  // ¥0.15/¥1.5 (≤128K)
      // ── Qwen Turbo ──
      { model_id: 'qwen-turbo',         capabilities: ['text', 'cheap'],                rpm: 200, cost: 0.001 },  // ¥0.3/¥0.6
      // ── QwQ (reasoning) ──
      { model_id: 'qwq-plus',           capabilities: ['text', 'thinking'],             rpm: 60,  cost: 0.002 },  // ¥1.6/¥4
      // ── QVQ (visual reasoning) ──
      { model_id: 'qvq-max',            capabilities: ['text', 'vision', 'thinking'],   rpm: 30,  cost: 0.006 },  // ¥8/¥32
      { model_id: 'qvq-plus',           capabilities: ['text', 'vision', 'thinking'],   rpm: 60,  cost: 0.002 },  // ¥2/¥5
      // ── Qwen VL (vision-language) ──
      { model_id: 'qwen3-vl-plus',      capabilities: ['text', 'vision', 'thinking'],   rpm: 30,  cost: 0.002 },  // ¥1/¥10 (≤32K)
      { model_id: 'qwen3-vl-flash',     capabilities: ['text', 'vision', 'thinking', 'cheap'], rpm: 60, cost: 0.001 },  // ¥0.15/¥1.5 (≤32K)
      { model_id: 'qwen-vl-max',        capabilities: ['text', 'vision'],               rpm: 30,  cost: 0.002 },  // ¥1.6/¥4
      { model_id: 'qwen-vl-plus',       capabilities: ['text', 'vision', 'cheap'],      rpm: 60,  cost: 0.001 },  // ¥0.8/¥2
      // ── Qwen Coder ──
      { model_id: 'qwen3-coder-plus',   capabilities: ['text', 'thinking'],             rpm: 30,  cost: 0.004 },  // ¥4/¥16 (≤32K)
      { model_id: 'qwen3-coder-flash',  capabilities: ['text', 'thinking'],             rpm: 60,  cost: 0.002 },  // ¥1/¥4 (≤32K)
      // ── Qwen Long ──
      { model_id: 'qwen-long',          capabilities: ['text'],                         rpm: 60,  cost: 0.001 },  // ¥0.5/¥2
      // ── Third-party on DashScope ──
      { model_id: 'deepseek-v3.2',      capabilities: ['text'],                         rpm: 60,  cost: 0.001 },  // ¥2/¥3
      { model_id: 'deepseek-r1',        capabilities: ['text', 'thinking'],             rpm: 30,  cost: 0.003 },  // ¥4/¥16
    ],
  },
  {
    ...
    html += '<div class="stg-models-section">' +
      '<div class="stg-models-header">' +
        ...
        '<div class="stg-models-actions">' +
          (_findMatchingTemplate(p) ? '<button class="stg-btn-add stg-sync-btn" onclick="_syncProviderFromTemplate(' + pi + ')" title="从模板同步新模型">⟳ 同步模板</button>' : '') +
          '<button class="stg-btn-add stg-refresh-btn" onclick="_refreshAllModels(' + pi + ')" title="重新发现所有模型并更新定价（保留自定义别名和 RPM）">🔄 刷新全部</button>' +
          '<button class="stg-btn-add stg-discover-btn" onclick="_discoverModels(' + pi + ')" title="从 /v1/models 接口自动发现模型">🔍 自动发现</button>' +
...
/**
 * Sync a provider with its matching template — adds any new models that
 * exist in the template but not yet in the provider's model list.
 */
async function _syncProviderFromTemplate(provIdx) {
  await _loadExternalProviderTemplates();
  var p = _stgProviders[provIdx];
  if (!p) return;
  ...
  if (!tpl) {
    alert('未找到匹配的模板。此服务商可能是手动创建的。');
    return;
  }

  // Sync balance_url from template if provider doesn't have one yet
  if (!p.balance_url && tpl.balance_url) {
    p.balance_url = tpl.balance_url;
  }
  ...
  for (var i = 0; i < tpl.models.length; i++) {
    var tm = tpl.models[i];
    if (!existing.has(tm.model_id)) {
      p.models.push({
        model_id: tm.model_id,
        aliases: tm.aliases || [],
        capabilities: tm.capabilities || ['text'],
        rpm: tm.rpm || 30,
        cost: tm.cost || 0.01,
        thinking_default: (tm.capabilities || []).indexOf('thinking') >= 0,
      });
      existing.add(tm.model_id);
      added++;
    }
  }

  if (added === 0) {
    alert('模型已是最新 — 模板中没有新模型需要同步。');
    return;
  }
  ...
  alert('✅ 从「' + tpl.name + '」模板同步了 ' + added + ' 个新模型。\n点击"保存"以应用更改。');
}
...
async function _discoverModels(provIdx) {
  ...
  var btn = card ? card.querySelector('.stg-discover-btn') : null;
...
// ── Refresh All Models (re-discover + update pricing, preserve user customizations) ──

/**
 * Refresh all models for a provider: re-discover from /v1/models,
 * update pricing/capabilities, add new models, but preserve user
 * customizations (aliases, RPM overrides) on existing models.
 */
async function _refreshAllModels(provIdx) {
  var p = _stgProviders[provIdx];
  if (!p) return;
  ...
  if (!baseUrl) {
    alert('请先设置 API 地址 (Base URL)');
    return;
  }
  if (!apiKey) {
    alert('请先添加至少一个 API 密钥');
    return;
  }
  ...
  var cards = document.querySelectorAll('.stg-provider-card');
  var card = cards[provIdx];
  var btn = card ? card.querySelector('.stg-refresh-btn') : null;
  var oldText = btn ? btn.textContent : '';
  if (btn) {
    btn.disabled = true;
    btn.textContent = '⏳ 刷新中…';
  ...
  try {
    ...
    if (ct.indexOf('application/json') < 0) {
      var txt = await resp.text();
      alert('刷新失败 (HTTP ' + resp.status + '): ' + txt.substring(0, 200));
      return;
    }
    var data = await resp.json();
    ...
    // Build a lookup of existing user models for merge
    if (!p.models) p.models = [];
    var existingByMid = {};
    for (var i = 0; i < p.models.length; i++) {
      existingByMid[p.models[i].model_id] = p.models[i];
    }
    ...
    for (var d = 0; d < discovered.length; d++) {
      var dm = discovered[d];
      var existing = existingByMid[dm.model_id];
      if (existing) {
        // Update pricing and capabilities, preserve user customizations
        existing.cost = dm.cost;
        existing.capabilities = dm.capabilities;
        existing.thinking_default = dm.thinking_default;
        // Don't overwrite: aliases (user may have added custom ones), rpm (user may have tuned)
        mergedModels.push(existing);
        delete existingByMid[dm.model_id];
        updated++;
      } else {
        // New model from API
    ...
    // Keep any user-added models that weren't found in the API response
    // (they may be custom/fine-tuned models the user added manually)
    var kept = 0;
    for (var mid in existingByMid) {
      mergedModels.push(existingByMid[mid]);
      kept++;
    }

    p.models = mergedModels;
    _renderProvidersTab();
    var newCards = document.querySelectorAll('.stg-provider-card');
    if (newCards[provIdx]) newCards[provIdx].classList.add('expanded');

    alert('✅ 刷新完成：' + updated + ' 个已更新，' + added + ' 个新增' +
          (kept > 0 ? '，' + kept + ' 个自定义模型保留' : '') +
          '。\n点击「保存」以应用更改。');

  } catch (e) {
    alert('刷新出错: ' + e.message);
  } finally {
    if (btn) {
      btn.disabled = false;
      btn.textContent = oldText;
