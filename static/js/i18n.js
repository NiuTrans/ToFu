...
var _i18n = {
  ...
  'settings.paperInsightModule': { zh: '论文阅读：洞察二遍', en: 'Paper Reading: Insight Pass' },
  'settings.paperInsightModuleDesc': { zh: '报告生成后追加一轮「洞见与灵感」：这篇论文的赌注、与你读过的工作的联系、有依据的观点、开放问题与挑衅追问。会带来额外的模型调用费用（在报告底部的费用分解中可见）；API/无头调用默认关闭。', en: 'After each report, adds an insight pass: the paper\'s bet, connections to your reading, a grounded take, open problems and provocations. Bills extra model calls (visible in the report\'s cost breakdown); OFF by default for headless/API callers.' },
  'settings.paperCheckpointsModule': { zh: '论文阅读：随堂检查卡', en: 'Paper Reading: Checkpoint Cards' },
  'settings.paperCheckpointsModuleDesc': { zh: '为报告的每个大节生成一道自测题（翻面卡），用主动回忆固化理解。会产生少量额外模型调用。', en: 'Generates one self-test flip card per major report section — active recall to lock in understanding. Bills a small extra model call.' },
  ...
  'local.desktopRemote': { zh: 'Tofu 运行在远程服务器上。在你自己的电脑安装桌面版，再把 6 位配对码填进它的首次启动：', en: 'Tofu runs on a remote server. Install the desktop app on your own machine, then type the 6-digit pairing code into its first-run dialog:' },
  ...
  'local.agentStepPair': { zh: '② 点「配对这台电脑」（6 位码，可复制），填进受控端首次启动：', en: '2. Click “Pair this computer” (a 6-digit code you can copy) and type it into the agent\'s first-run dialog:' },
  'local.pairBtn': { zh: '配对这台电脑', en: 'Pair this computer' },
  'local.pairHint': { zh: '把这 6 位数字填进受控端首次启动 —— 它自己找服务器并完成配对（无需地址、无需隧道）。', en: 'Type these 6 digits into the agent\'s first-run dialog — it finds the server and pairs by itself (no address, no tunnel).' },
  'local.pairCopied': { zh: '配对码已复制 —— 粘贴到受控端首次启动', en: 'Pairing code copied — paste it into the agent\'s first-run dialog' },
  'local.pairExpires': { zh: '配对码 {mm}:{ss} 后过期', en: 'Code expires in {mm}:{ss}' },
  'local.pairExpired': { zh: '配对码已过期 —— 再点一次生成新码', en: 'Code expired — click again for a fresh one' },
  'local.connectLineToggle': { zh: '高级：连接行（配对码不可用时兜底）', en: 'Advanced: connect line (fallback when pairing is unavailable)' },
  ...
  'local.awaitingAgent': { zh: '已发出配对凭证，等待受控端连入……它会自己寻找服务器并自动重试（本机 → 局域网 → 自动隧道），一般一分钟内变绿；迟迟未连上时，在受控端托盘选「连接到另一个 Tofu…」用新配对码重配一次。', en: 'Credentials issued, waiting for the agent… it finds the server and retries by itself (this machine → LAN → automatic tunnel), usually green within a minute. If it stays red: on the agent tray choose "Connect to a different Tofu…" and re-pair with a fresh code.' },
  ...
  'paper.focusBtn': { zh: '专注', en: 'Focus' },
  'paper.focusTitle': { zh: '专注模式:一次只读一段,j/k 移动,Esc 退出', en: 'Focus mode: one block at a time, j/k to move, Esc to exit' },
  ...
  'paper.xpSkim': { zh: '速览', en: 'Skim' },
  'paper.xpSkimFull': { zh: '全文', en: 'Full' },
  'paper.xpSkimTitle': { zh: '速览模式：每节只留首段与要点', en: 'Skim read: first paragraph + callouts per section' },
