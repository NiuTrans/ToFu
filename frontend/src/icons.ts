 ...
 *
 * Glyphs are trusted Lucide-style path literals that inherit text color. The
 * renderer owns the SVG wrapper; callers select a semantic name and never
 * inject path markup.
 *
 * Responsibility: render trusted, generic UI glyphs and welcome capability
 * pills without reading DOM, globals, or retained runtime state.
 * Entry points: `iconHtml`, `statusDotIconHtml`, `hasIcon`, and
 * `welcomePillsHtml`. Dependencies: the injected generated i18n translator.
 */
...
const ICON_PATHS: Readonly<Record<string, string>> = {
    ...
    bot: '<path d="M12 2v4"/><path d="M12 18v4"/><rect x="4" y="8" width="16" height="10" rx="2"/><path d="M9 13h.01"/><path d="M15 13h.01"/>',
  ...
  /** Return trusted inline SVG for a named glyph, or empty for an unknown key. */
  export function iconHtml(
    ...
    const style = dim + ';vertical-align:-0.125em;flex-shrink:0'
      + (extraStyle ? `;${extraStyle}` : '');
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" ' +
      'stroke-linecap="round" stroke-linejoin="round" style="' + style + '">' + inner + '</svg>';
  }

  /** Return a filled status dot using a named status color or explicit CSS color. */
  export function statusDotIconHtml(color?: string, size = 9): string {
    const fill = STATUS_DOT_COLORS[color ?? ''] || color || STATUS_DOT_COLORS.grey;
    const d = size;
  ...
  /** Public capability probe for literal-callsite and feature-boundary checks. */
  ...
  /** Render the fixed welcome capability list through the injected translator. */
  export function welcomePillsHtml(translate: Translator): string {
    return WELCOME_PILLS.map(function (p) {
      return '<span class="feature-pill">' + iconHtml(p[0], 13) +
        '<span data-i18n="' + p[1] + '">' + translate(p[1]) + '</span></span>';
    }).join('');
  }
