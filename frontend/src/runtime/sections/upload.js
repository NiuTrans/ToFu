...
// Key: 'tofu_vlm_pending' → JSON array of {name, text, pages, textLength,
// isScanned, method, vlmStatus, vlmTaskId, vlmProgress, vlmError}
 ...
 * via runtimeScope._uploadShrinkPolicy. The client-side pass only exists to cap
...
function compressImage(file, userMaxWidth) {
  ...
  const policy = (typeof window !== 'undefined' && runtimeScope._uploadShrinkPolicy) || {};
...
/** Wait until optimistic image chips have durable payloads before capture. */
async function _waitForImageProcessing() {
  const deadline = Date.now() + 2 * 60 * 1000;
  while (pendingImages.some((image) => image?._status === 'processing')) {
  ...
  for (const image of pendingImages) {
    if (image && image._status !== 'processing' && (image.base64 || image.url)) {
      ready.push(image);
      continue;
    }
    if (image?._objectUrl) {
      try { URL.revokeObjectURL(image._objectUrl); } catch (e) { /* ignore */ }
    }
  }
  pendingImages = ready;
  if (pendingImages.length < before) {
    const message = t('upload.imageSkipped');
    if (typeof showToast === 'function') showToast(message, 'warning');
    else debugLog(message, 'warn');
    renderImagePreviews();
  }
}
        ...
        vlmS === "parsing"
          ...
          : vlmS === "done"
            ...
            : vlmS === "failed" || vlmS === "timeout"
              ? `<div class="pdf-vlm-badge failed" title="${escapeHtml(pdf.vlmError || '')}">${Icon('zap',11)} VLM</div>`
              : "";
      const methodLabel = pdf.method === "vlm" ? "VLM" : "TEXT";
      const docIcon = pdf._docIcon || Icon('file',22);
      return `<div class="img-preview pdf-text-card" data-tofu-action="previewPendingPdfText(${i})"><div class="pdf-text-card-inner"><div class="pdf-text-icon">${docIcon}</div><div class="pdf-text-info"><div class="pdf-text-name" title="${escapeHtml(pdf.name)}">${escapeHtml(pdf.name.length > 20 ? pdf.name.slice(0, 18) + "…" : pdf.name)}</div><div class="pdf-text-meta">${pdf.pages}p · ${sizeStr}${badge}</div>${vlmBadge}</div></div><button class="remove-img" data-tofu-action="event.stopPropagation();removePdfText(${i})">✕</button><div class="img-size">${methodLabel}</div></div>`;
    })
    .join("");
  html += pendingImages
    .map((img, i) => {
      ...
      return `<div class="img-preview${isPdf ? " pdf-page" : ""}${processing ? " img-processing" : ""}"${dragAttr} data-img-idx="${i}" ${tip ? `title="${tip}"` : ""}  data-tofu-action="previewPendingImage(${i})"><img src="${img.preview}" alt="preview" draggable="false">${overlay}${srcLabel ? `<div class="pdf-badge">${srcLabel}</div>` : ""}<button class="remove-img" data-tofu-action="event.stopPropagation();removeImage(${i})">✕</button><div class="img-size">${processing ? t('upload.processing') || '…' : label}</div></div>`;
    })
    .join("");
  html += pendingVideos
    .map((v, i) => {
      ...
      const click = (!busy && !failed && v.video_url)
        ? ` data-tofu-action="openVideoUrl('${String(v.video_url).replace(/'/g, "\\'")}')"` : '';
      const overlay = busy
        ? `<div class="img-processing-overlay"><div class="img-processing-spinner"></div></div>` : '';
      return `<div class="img-preview video-chip${failed ? ' video-failed' : ''}"${click} title="${escapeHtml(v.name)}">${thumb}${overlay}<div class="pdf-badge">VIDEO</div><button class="remove-img" data-tofu-action="event.stopPropagation();removeVideo(${i})">✕</button><div class="img-size">${escapeHtml(meta)}</div></div>`;
    })
    .join("");
  const targetEl = document.getElementById("imagePreviews");
...
runtimeScope._vlmParseEntry = async function(file, entry, isAlive, onUpdate) {
  if (!entry) return;
  entry.vlmStatus = "parsing";
  onUpdate();
  _vlmSaveState();
