/* ===== migrated source: main/main_folders_mobile.js ===== */
/* ═══════════════════════════════════════════════════════════════════
 ...
 * translate, browser, imageGen, humanGuidance, swarm, and Agent Mode).
...
function updateMobileSheet() {
  ...
  const agentMode = resolveAgentMode(planMode, autopilotEnabled);
  document.querySelectorAll('.mobile-agent-mode-item').forEach((item) => {
    const selected = !activeFlow && item.dataset.mode === agentMode;
    item.classList.toggle('active', selected);
    item.setAttribute('aria-checked', selected ? 'true' : 'false');
  });
  const workflowItem = document.getElementById('mobileFlow');
  const workflowLabel = document.getElementById('mobileFlowLabel');
  if (workflowItem) workflowItem.classList.toggle('active', !!activeFlow);
  if (workflowLabel) {
    if (activeFlow && typeof _flowDisplayName === 'function') {
      workflowLabel.removeAttribute('data-i18n');
      workflowLabel.textContent = _flowDisplayName(activeFlow);
    } else {
      workflowLabel.dataset.i18n = 'mobile.workflowDesc';
      workflowLabel.textContent = typeof t === 'function'
        ? t('mobile.workflowDesc') : 'mobile.workflowDesc';
    }
  }
  ...
  const map = {
     ...
     * frontend/src/runtime/local-control.js); the merged entry is .active when either
     * capability is on. */
    mobileLocalControl: "localControlToggle"
  };
  let activeCount = agentMode === 'standard' && !activeFlow ? 0 : 1;
...
function updateMobileContext() {
  ...
  const summary = (typeof runtimeScope.contextUsageSummary === "function")
    ? runtimeScope.contextUsageSummary() : null;
  ...
  const busy = Boolean(conv && typeof convIsBusy === 'function' && convIsBusy(conv));
  if (compactItem) {
...
if (typeof window !== "undefined") runtimeScope.updateMobileContext = updateMobileContext;
...
function _mobileCompactNow() {
  ...
  if (!cid) return;
  closeMobileSheet();
  if (typeof runtimeScope.runManualCompaction === "function") {
    runtimeScope.runManualCompaction(cid);
  } else {
    console.warn("[mobileCompact] runManualCompaction not loaded");
  }
}
if (typeof window !== "undefined") runtimeScope._mobileCompactNow = _mobileCompactNow;
...
(function initMobileKeyboardHandler() {
  ...
  function onViewportResize() {
    ...
    if (!growing) {
      ...
      if (_wasNearBottom || (typeof getActiveConv === 'function'
          && typeof convIsBusy === 'function' && convIsBusy(getActiveConv()))) {
...
if (typeof window !== 'undefined') runtimeScope.refreshInputSendHint = refreshInputSendHint;
