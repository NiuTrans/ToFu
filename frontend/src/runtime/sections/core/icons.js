/* ===== migrated source: core/icons.js ===== */
/* ═══════════════════════════════════════════════════════════════════
   core/icons.js — central inline-SVG icon registry (Lucide-based).

   §3.4 compliance: the UI uses SVG icons, NEVER emoji. This module is the
   single source of truth for generic-concept glyphs rendered from JS.

   Usage:
     Icon('rocket')          → '<svg …>…</svg>' at default 1em (inherits color)
     Icon('rocket', 14)      → fixed 14×14 px
     Icon('rocket', 14, 'vertical-align:-2px')  → extra inline style

   All glyphs are stroke="currentColor" Lucide paths (viewBox 0 0 24 24),
   so they inherit the surrounding text color. Injected via innerHTML only
   (the strings are static and trusted — no user input).

   This file is concatenated by Vite's module graph near the front of the
   bundle (right after i18n.js) so every later module can call Icon().
   ═══════════════════════════════════════════════════════════════════ */

(function () {
  'use strict';

  // name → inner SVG markup (paths/circles/etc., no <svg> wrapper).
  var _PATHS = {
    ...
    alarm: '<circle cx="12" cy="13" r="8"/><path d="M5 3 2 6"/><path d="m22 6-3-3"/><path d="M6.38 18.7 4 21"/><path d="M17.64 18.67 20 21"/><path d="M12 10v3h3"/>',
    ...
    messagePlus: '<path d="M22 17a2 2 0 0 1-2 2H6l-4 4V5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2z"/><path d="M12 7v6"/><path d="M9 10h6"/>',
    ...
    fileSheet: '<path d="M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z"/><path d="M14 2v5a1 1 0 0 0 1 1h5"/><path d="M8 13h2"/><path d="M14 13h2"/><path d="M8 17h2"/><path d="M14 17h2"/>',
    ...
    // Lucide refresh-cw — identical glyph to 'refresh' above, kept under its
    // canonical name so callers don't guess which alias exists.
  ...
  // because these convey state, not text flow.
  var _DOTS = {
    green:  '#22c55e',
    yellow: '#eab308',
    red:    '#ef4444',
   ...
   * @param {string} name  key in _PATHS
   * @param {number|string} [size]  px size (number) or omit for 1em
   * @param {string} [extraStyle]  extra inline CSS appended to style=""
   */
  function Icon(name, size, extraStyle) {
    var inner = _PATHS[name];
    if (!inner) return '';
    var dim = (size == null) ? 'width:1em;height:1em' : ('width:' + size + 'px;height:' + size + 'px');
    var style = dim + ';vertical-align:-0.125em;flex-shrink:0' + (extraStyle ? ';' + extraStyle : '');
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" ' +
      'stroke-linecap="round" stroke-linejoin="round" style="' + style + '">' + inner + '</svg>';
  }

  /** Colored status dot (filled circle). @param {string} color key in _DOTS or a CSS color. */
  function IconDot(color, size) {
    var fill = _DOTS[color] || color || _DOTS.grey;
    var d = (size == null) ? 9 : size;
    return '<svg viewBox="0 0 24 24" width="' + d + '" height="' + d + '" fill="' + fill +
  ...
  // welcome render sites (index.html static markup and newChat()) so the pill
  ...
  runtimeScope.Icon = Icon;
  runtimeScope.IconDot = IconDot;
  runtimeScope._welcomePillsHtml = _welcomePillsHtml;
})();

// Classic scripts obtained these names from the global object. ESM keeps the
// icon registry private, so retain one explicit lexical bridge for the legacy
// renderers that still share this module.
const Icon = runtimeScope.Icon;
const IconDot = runtimeScope.IconDot;
const _welcomePillsHtml = runtimeScope._welcomePillsHtml;
