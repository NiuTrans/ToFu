...
function _timerWatcherToggle(el, ev) {
  ...
  const block = el && el.closest ? el.closest('.timer-watcher-block') : null;
  const body = block && block.querySelector ? block.querySelector('.timer-watcher-body') : null;
  if (!body) return;
  const expanded = body.classList.toggle('expanded');
  el.setAttribute('aria-expanded', String(expanded));
}

/* Checklist revisions collapse to one structured progress card. The display
 * projection remains in core because both timeline paths compact revisions;
 * only the optional rich card renderer lives here. */
function _renderTodoBlock(round, svg, q, badgeHtml) {
  ...
  const updateCount = Number(meta.todoDisplayUpdateCount || round.todoDisplayUpdateCount
    || meta.todoUpdateCount || 1);
  const breadcrumbs = Array.isArray(meta.todoBreadcrumbs) ? meta.todoBreadcrumbs : [];
  const displayHistory = Array.isArray(meta.todoDisplayHistory) ? meta.todoDisplayHistory : [];
  ...
  const _ICON = {
    ...
    blocked: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3 2.8 20h18.4L12 3Z"/><path d="M12 9v5"/><circle cx="12" cy="17" r=".7" fill="currentColor"/></svg>',
  ...
  for (const item of todos) {
    ...
    const st = (item.status === "completed" || item.status === "in_progress" || item.status === "blocked") ? item.status : "pending";
  ...
  const countChip = total
    ...
    : "";
  const revisionsChip = updateCount > 1
    ? `<span class="ptool-todo-revisions" title="${escapeHtml(_t("todo.historyTip", "Earlier checklist revisions remain available in task debug history."))}">${escapeHtml(_t("todo.updates", { n: updateCount }))}</span>`
    : "";
  const breadcrumbHtml = breadcrumbs.length > 1
    ? `<div class="ptool-todo-breadcrumbs">${breadcrumbs.map((b, i) =>
        `<span${i === breadcrumbs.length - 1 ? ' class="is-active"' : ''}>${escapeHtml(b.label || b.checklist_id || "")}</span>`
      ).join('<i>›</i>')}</div>`
    : "";
  let stateNotice = "";
  ...
  return `<details class="ptool-todo-block" data-rn="${round.roundNum}">
       ...
       <div class="ptool-todo-body">${breadcrumbHtml}${stateNotice}<div class="ptool-todo-list">${rows}</div>${historyHtml}</div>
     </details>`;
}
...
function _renderConvDigest(cd) {
  ...
  for (const m of msgs) {
    ...
    //    turnId). Rendered ONLY when the digest is a raw read AND the field is
    ...
    if (isRaw) {
      ...
      if (m.turnId) {
        hints.push(`<span class="ptool-convdigest-metachip ptool-convdigest-meta-id" title="${escapeHtml(
          _t("convDigest.metaTurnId", "turn id"))}">${escapeHtml(String(m.turnId))}</span>`);
      }
    }
...
const _CONV_META_ROUTINE_READS = new Set([
  ...
  /* get_conversation joined the collapsed reads 2026-08-27 (owner directive):
   * a digest card is still a tool RESULT, and results stay folded until the
   * user chooses to read them — the summary row keeps the title + a
   * "{n} messages" count chip so nothing is hidden behind an anonymous row. */
  "get_conversation",
]);
...
function _renderTimerWatcherBlock(round, svg) {
  const recovered = _timerRecoveryPresentation(round);
  const polls = recovered.polls;
  const triggered = recovered.triggered;
  const isActive = round.status === "searching" && !triggered;
  ...
  if (instr || cmd || interval) {
    ...
    if (instr) {
      ...
      const toggle = LONG
        ? ` data-tofu-action="event.stopPropagation();var v=document.getElementById('${valId}');v.classList.toggle('expanded');this.querySelector('.timer-meta-more').textContent=v.classList.contains('expanded')?this.querySelector('.timer-meta-more').getAttribute('data-less'):this.querySelector('.timer-meta-more').getAttribute('data-more');"`
        : "";
      const moreLink = LONG
  ...
  for (const p of visible) {
    ...
    const toggleAttr = hasDetail
      ? ` data-tofu-action="event.stopPropagation();var d=this.nextElementSibling;if(d){d.classList.toggle('expanded');this.classList.toggle('expanded');}"`
      : "";
    const caret = hasDetail ? `<span class="timer-poll-caret">${Icon('chevronDown', 12, 'transform:rotate(-90deg)')}</span>` : `<span class="timer-poll-caret-spacer"></span>`;
  ...
  return `<div class="timer-watcher-block ${headerCls}" data-rn="${round.roundNum}">
       <div class="timer-watcher-header" role="button" tabindex="0" aria-expanded="${expandedByDefault ? 'true' : 'false'}" data-tofu-action="_timerWatcherToggle(this,event)">
         ...
         ${_rowRightControls(round)}
         <span class="timer-toggle">${Icon('chevronDown', 14)}</span>
       </div>
