...
const _PT_CHEVRON_RIGHT_SVG = '<svg viewBox="0 0 16 16" width="16" height="16" fill="none" aria-hidden="true"><path d="m6.5 5 3 3-3 3" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';

function _plainToolStatus(value, fallback) {
...
function _isRoundSearch(round) {
  return round.toolName === "web_search" || round.toolName === "search_knowledge";
}
function _isRoundToolSearch(round) {
  return round.toolName === "search_tools";
}
...
/* Canonical browser family. This list exactly mirrors BROWSER_TOOL_NAMES,
 * ADVANCED_BROWSER_TOOL_NAMES, and PAGE_PREVIEW_TOOL_NAMES; the parity test
 * prevents a new tool from silently falling back to generic UI. */
...
/* Tools whose prose belongs in the collapsible conversation-metadata block. */
const _CONV_META_TOOLS = new Set([
  ...
  // Read-only compatibility: persisted rounds may still carry this tool.
  "project_charter_commit",
  "list_conversations", "get_conversation",
  "project_peer_status", "project_feed_read",
  "project_message", "project_intervene",
  // Integration tools return prose that needs the same collapsible block.
  "integration_checkpoint", "integration_submit", "integration_status",
...
/* Motion/produce family; tests/test_motion_video_display.py pins backend
 * parity so result bodies never degrade to the generic row. */
...
const _TOOL_DISPLAY = {
  ...
  search_knowledge: { icon: "", label: "Knowledge", color: "#22c55e" },
  search_tools:  { icon: "", label: "Tool Search", color: "#818cf8" },
...
function _getRoundIcon(round) {
  ...
  if (_isRoundToolSearch(round)) return "search_tools";
  if (round.toolName === "search_knowledge") return "search_knowledge";
...
function _renderHumanGuidanceCard(round, svg, opts) {
  ...
  let inputHtml = '';
  if (expired) {
    /* Expired (turn settled before an answer arrived — e.g. server restart):
     * the backend pending request is gone forever, so render the question and
     * options READ-ONLY. No buttons, no data-tofu-action, no textarea —
     * clicking must be impossible rather than fail with a 404. */
    if (respType === 'choice' && options.length > 0) {
      const optRowsHtml = options.map((opt, i) => {
        const displayLabel = opt._translatedLabel || opt.label || `Option ${i + 1}`;
        ...
        const descHtml = displayDesc
          ...
          : '';
        return `<div class="hg-option-card hg-option-static">
                  <div class="hg-opt-label">${escapeHtml(displayLabel)}</div>
                  ${descHtml}
                </div>`;
      }).join('');
      inputHtml = `<div class="hg-options-grid">${optRowsHtml}</div>`;
    }
  } else if (respType === 'choice' && options.length > 0) {
    ...
    const optCardsHtml = options.map((opt, i) => {
      ...
      // escapeHtml the JSON.stringify output so double-quotes don't break data-tofu-action="..." attribute
      ...
      return `<button class="hg-option-card" data-gid="${gid}" data-label="${escapeHtml(origLabel)}"
                      data-tofu-action="event.stopPropagation();submitHumanGuidanceChoice('${gid}',${safeJsonLabel})">
                <div class="hg-opt-label">${escapeHtml(displayLabel)}</div>
                ${descHtml}
              </button>`;
    }).join('');
    ...
    inputHtml = `<div class="hg-freetext-wrap">
      <textarea class="hg-textarea" id="hg-input-${gid}" rows="3"
                ...
                data-tofu-action-keydown="if(event.key==='Enter'&&(event.ctrlKey||event.metaKey)){event.preventDefault();submitHumanGuidanceFreeText('${gid}')}"></textarea>
      <div class="hg-freetext-actions">
        <button class="hg-submit-btn" data-tofu-action="event.stopPropagation();submitHumanGuidanceFreeText('${gid}')">
  ...
  return `<div class="hg-card${expired ? ' hg-expired' : ''}" data-gid="${gid}">
    <div class="hg-header">
      
      <span class="hg-title">${escapeHtml(t('project.hgPanelTitle'))}</span>
      <span class="hg-badge">${escapeHtml(t(expired ? 'project.hgExpired' : 'project.hgWaitingReply'))}</span>
    </div>
...
function _renderRejectedToolLine(round, svg) {
  ...
  const shortReason = firstLine.length > 160 ? firstLine.slice(0, 160) + "…" : firstLine;
...
function _renderVerticalCard(v) {
  ...
  const rows = items.slice(0, 12).map(it => {
    ...
    if (it.upvotes != null && it.upvotes !== "")
      meta.push(`<span class="vertical-meta-pill" title="Upvotes"><span class="vertical-meta-icon">${Icon('arrowRight', 12, 'transform:rotate(-90deg)')}</span>${escapeHtml(String(it.upvotes))}</span>`);
    if (it.citations != null && it.citations !== "")
      meta.push(`<span class="vertical-meta-pill" title="Citations"><span class="vertical-meta-icon">${Icon('refreshCw', 12)}</span>${escapeHtml(Number(it.citations).toLocaleString())}</span>`);
    if (it.year)
...
/* ── MOVED to ui/tool_rounds_rich.js (the migrated lazy-module graph, Epic-E  sub-4, 2026-08-01) — rich conv-meta + timer-watcher renderers; core dispatch is typeof-guarded and degrades to the generic ptool-line until the lazy ESM domain lands. ── */

/* Localize the inspect_image ops chip. The backend (lib/file_reader/_image.py) builds
...
function _renderUnifiedToolLine(round, isSearching) {
  ...
  // Human Guidance — LLM is asking the user a question. A leftover
  //   awaiting_human round on a SETTLED turn (marked via round._turnSettled by
  //   renderToolBlockHtml) is unanswerable — the pending request died with the
  //   task/process — so it renders as a read-only expired card.
  if (round.status === "awaiting_human" && round.guidanceId) {
    return _renderHumanGuidanceCard(round, svg, { expired: !!round._turnSettled });
  }
  ...
  //   line until the lazy ESM domain lands; Epic-E sub-4)
  ...
  // Local Tool Search — show exactly which original tools became
  //   discoverable. These are native tool names, not execute_tools wrappers.
  const toolSearchHtml = _renderToolCatalogSearch(round, ctx);
  ...
  const genericResultHtml = _renderGenericToolResultBlock(round, ctx, badgeHtml, compactionLabelHtml);
...
function _renderPendingApprovalBlock(round, ctx) {
  ...
  if (Array.isArray(ameta.riskFields) && ameta.riskFields.length) {
    // Enrichers declare risk fields here instead of adding tool-specific shapes.
  ...
  return `<div class="ptool-pending-wrap">
         ...
         <div class="ptool-approval-btns">
           <button class="ptool-approve-btn" data-tofu-action="event.stopPropagation();resolveWriteApproval('${aid}',true)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg> Approve</button>
           <button class="ptool-reject-btn" data-tofu-action="event.stopPropagation();resolveWriteApproval('${aid}',false)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> Reject</button>
         </div>
       </div>`;
}
...
/* Display-only signal: run_command still owns the command/result protocol;
 * this small chip only tells the human that a filesystem grep segment is
 * executing through the bounded grep_search backend. */
function _renderGrepInterceptBadge(round, meta) {
  const intercepted = !!(round && round.grepSearchIntercepted)
    || !!(meta && meta.grepSearchIntercepted);
  if (!intercepted) return '';
  const _tf = (typeof t === 'function') ? t : (_k, d) => d;
  const label = _tf('toolCmd.grepSearchIntercepted', 'grep_search takeover');
  return `<span class="ptool-grep-intercept">${escapeHtml(label)}</span>`;
}
  ...
  if (round.toolName === "run_command" || round.toolName === "code_exec") {
    ...
    return `<div class="ptool-cmd-block ptool-cmd-running${cmdOpen ? ' cmd-open' : ''}"${cmdCollapsible ? ` data-cmd-key="${escapeHtml(cmdKey)}"` : ''}>
           <div class="ptool-cmd-header">
             ...
             ${cmdRootPill}
             ${descInlineHtml}
             ${_renderGrepInterceptBadge(round)}
...
/* Long described commands start collapsed. Expansion is keyed by toolCallId
 * so progress and timeline rerenders preserve the user's choice. */
...
function _cmdDescInline(desc, collapsible) {
  ...
  return `<span class="ptool-cmd-desc-inline ptool-cmd-desc-toggle" title="${esc}" data-tofu-action="_cmdBodyToggle(this,event)">${esc}</span>`;
}
...
function _renderCmdDoneBlock(round, ctx) {
  ...
  if (round.toolName !== "run_command" && round.toolName !== "code_exec") return "";

  let args = {};
  try {
    args = typeof round.toolArgs === "string" ? JSON.parse(round.toolArgs) : (round.toolArgs || {});
  } catch (_e) { /* malformed legacy args — the persisted query still names the command */ }
  const cmdRaw = meta.command ?? args.command ?? round.query ?? "";
  const description = meta.description ?? args.description ?? "";
  const rawContent = typeof round.toolContent === "string" ? round.toolContent : "";
  const exitMatch = rawContent.match(/\[exit code: (-?\d+)\]\s*$/);
  let timedOut = meta.timedOut === true || rawContent.indexOf("[Command timed out]") !== -1;
  let interrupted = meta.interrupted === true || rawContent.indexOf("[Command interrupted by") !== -1;
  const hasStructuredExit = meta.exitCode != null || meta.notRun === true;
  let exitCode = meta.exitCode;
  if (exitCode == null) {
    exitCode = timedOut ? "timeout" : (exitMatch ? exitMatch[1] : "?");
  }
  let output = meta.output;
  if (output == null && rawContent) {
    output = rawContent;
    const prefix = `$ ${cmdRaw}\n`;
  ...
  const cmdCollapsible = _cmdCollapsible(description, cmdRaw);
  const cmdKey = cmdCollapsible ? _cmdBodyKey(round) : '';
  const cmdOpen = cmdCollapsible && cmdKey && _cmdBodyExpanded.has(cmdKey);
  const descInlineHtml = _cmdDescInline(description, cmdCollapsible);
  ...
  const notRun = meta.notRun === true || exitCode === "not-run"
    || (!hasStructuredExit && !!rawContent && !exitMatch && !timedOut && !interrupted);
  if (notRun && exitCode === "?") exitCode = "not-run";
  const isOk = !notRun && (exitCode === "0" || exitCode === 0);
  /* Per-command interrupt (): the command was killed by the
    ...
    : interrupted
      ...
      : timedOut
        ...
        : isOk
          ? "ptool-cmd-ok"
          : "ptool-cmd-err";
  const notRunBadge = meta.badge && !meta.recovered && meta.badge !== `exit ${exitCode}`
    ? meta.badge : "not run";
  const _tfCmd = (typeof t === 'function') ? t : (k, d) => d;
  /* Completion and success are separate backend-owned facts: only a real exit
   * code gets a finish chip; verdict retains exit/timeout/interrupted/not-run. */
  const hasRealExit = !notRun && !interrupted && !timedOut
    && exitCode !== "?" && exitCode != null;
  const finishChipHtml = hasRealExit
    ? `<span class="ptool-cmd-finish">${escapeHtml(_tfCmd('toolCmd.finished', 'finished'))}</span>`
    : "";
  const statusLabel = notRun
    ? escapeHtml(notRunBadge)
    : interrupted
      ? escapeHtml(_plainToolStatus(_tfCmd('toolCmd.interruptedBadge', 'interrupted'), 'interrupted'))
      : timedOut
        ? "timeout"
        : isOk
          ? "exit 0"
  ...
  const reason = notRun ? (meta.reason || output || rawContent) : "";
  const qrStripHtml = _renderQrStrip(meta);
  /* Done output is folded under the title-row disclosure. */
  const hasOutput = !notRun && !!output;
  const outputKey = hasOutput ? _cmdBodyKey(round) : '';
  const outputOpen = hasOutput && outputKey && _cmdOutputExpanded.has(outputKey);
  let outputHtml = "";
  if (notRun && reason) {
  ...
  const outChevron = hasOutput
    ? `<span class="ptool-cmd-outchev" aria-hidden="true">${_PT_CHEVRON_RIGHT_SVG}</span>` : '';
  return `<div class="ptool-cmd-block ${statusCls}${cmdOpen ? ' cmd-open' : ''}${hasOutput ? ' ptool-cmd-hasoutput' : ''}${outputOpen ? ' output-open' : ''}" data-rn="${round.roundNum}"${cmdCollapsible ? ` data-cmd-key="${escapeHtml(cmdKey)}"` : ''}${hasOutput ? ` data-output-key="${escapeHtml(outputKey)}"` : ''}>
         <div class="ptool-cmd-header"${hasOutput ? ` role="button" aria-expanded="${outputOpen ? 'true' : 'false'}" data-tofu-action="_cmdHeaderToggle(this,event)"` : ''}>
           ...
           ${cmdRootPill}
           ${descInlineHtml}
            ${_renderGrepInterceptBadge(round, meta)}
            <span class="ptool-cmd-chips">${finishChipHtml}<span class="ptool-cmd-status">${statusLabel}</span>${outChevron}</span>
            ${_rowRightControls(round)}
         </div>
...
function _renderBrowserExecJsBlock(round, ctx) {
  ...
  let outputHtml = "";
  if (out) {
    const _tfJs = (typeof t === 'function') ? t : (k, d) => d;
    outputHtml = `<div class="ptool-cmd-output-wrap">
           <button type="button" class="ptool-cmd-toggle" data-tofu-action="_cmdOutputToggle(this,event,'result')">${_PT_CHEVRON_RIGHT_SVG}${escapeHtml(_tfJs('toolCmd.showResult', 'Show result'))}</button>
...
// Local catalog search — a compact, always-visible set of matched native
// tool cards. Search metadata is projected by the backend; the private MCP
// indexing metadata and full schemas never enter this display structure.
function _renderToolCatalogSearch(round, ctx) {
  if (!_isRoundToolSearch(round) || round.status !== "done") return "";
  const { svg, q, results } = ctx;
  const matches = results.filter((item) => item && item.type === "tool_catalog_match");
  const total = Number.isFinite(Number(round.toolSearchTotal))
    ? Number(round.toolSearchTotal) : matches.length;
  const foundText = (typeof t === "function")
    ? String(t("toolSearch.found", { total, shown: matches.length }))
    : `${total} candidate matches · showing ${matches.length}`;
  const moreText = round.toolSearchNextCursor
    ? `<span class="ptool-tool-search-more">${escapeHtml(typeof t === "function" ? t("toolSearch.more") : "more candidates available")}</span>`
    : "";
  const failOpenText = round.toolSearchFailOpen
    ? `<span class="ptool-badge ptool-badge-warn">${escapeHtml(typeof t === "function" ? t("toolSearch.failOpen") : "full catalog restored")}</span>`
    : "";

  let cards = "";
  if (matches.length) {
    cards = matches.map((item) => {
      const args = Array.isArray(item.arguments) ? item.arguments : [];
      const visibleArgs = args.slice(0, 8).map((arg) => {
        const required = !!arg.required;
        const requiredMark = required ? '<span class="ptool-tool-arg-required">*</span>' : "";
        return `<span class="ptool-tool-arg${required ? " is-required" : ""}"><code>${escapeHtml(arg.name || "arg")}</code>${requiredMark}<span>${escapeHtml(arg.type || "value")}</span></span>`;
      }).join("");
      const overflow = args.length > 8
        ? `<span class="ptool-tool-arg ptool-tool-arg-more">+${args.length - 8}</span>` : "";
      const argHtml = (visibleArgs || overflow)
        ? `<div class="ptool-tool-search-args">${visibleArgs}${overflow}</div>` : "";
      const desc = item.snippet
        ? `<div class="ptool-tool-search-desc">${escapeHtml(item.snippet)}</div>` : "";
      return `<div class="ptool-tool-search-card">
        <div class="ptool-tool-search-card-head">
          <code class="ptool-tool-search-name">${escapeHtml(item.toolName || item.title || "tool")}</code>
          <span class="ptool-tool-search-namespace">${escapeHtml(item.namespace || "general")}</span>
        </div>
        ${desc}${argHtml}
      </div>`;
    }).join("");
  } else {
    cards = `<div class="ptool-tool-search-empty">${escapeHtml(typeof t === "function" ? t("toolSearch.none") : "No matching tools")}</div>`;
  }
  ...
  return `<div class="ptool-tool-search-block">
    <div class="ptool-line ptool-tool-search-header">
      <span class="ptool-icon">${svg}</span>
      <span class="ptool-text">${q}</span>
      <span class="ptool-tool-search-count">${escapeHtml(foundText)}</span>
      ${failOpenText}${moreText}${_rowRightControls(round)}
    </div>
    <div class="ptool-tool-search-results">${cards}</div>
  </div>`;
}
...
function _renderSearchRows(round, ctx) {
  ...
  if ((_isRoundSearch(round) || _isRoundFetch(round)) && results.length === 0) {
    ...
    const srcBadge = (round.cacheSource === 'prefetch' || round.cacheSource === 'cache')
      ? `<span class="ptool-badge" style="opacity:.55" title="Served from a ${round.cacheSource === 'prefetch' ? 'streaming prefetch' : 'dedup cache'} hit — the search ran earlier, not just now">${round.cacheSource}</span>`
      : "";
  ...
  if ((_isRoundSearch(round) || _isRoundFetch(round)) && results.length > 0) {
    ...
    if (eb && typeof eb === "object") {
      ...
      if (engines.length > 0) {
        ...
        engineBkdnHtml = `<div class="eb-section">
          <button type="button" class="eb-toggle" aria-expanded="false" data-tofu-action="event.stopPropagation();this.parentElement.classList.toggle('eb-expanded');this.setAttribute('aria-expanded',String(this.parentElement.classList.contains('eb-expanded')))"><span class="eb-icon">${svg}</span>Engine Sources <span class="eb-total">${totalRaw} raw → ${results.length} final</span><span class="eb-arrow">${Icon('chevronDown', 16, 'transform:rotate(-90deg)')}</span></button>
    ...
    return `<div class="ptool-results-block" data-rn="${round.roundNum}">
         <div class="ptool-line ptool-results-header" role="button" tabindex="0" aria-expanded="false" data-tofu-action="if(event.target.closest('.ri-tool-anchor'))return;event.stopPropagation();this.parentElement.classList.toggle('expanded');this.setAttribute('aria-expanded',String(this.parentElement.classList.contains('expanded')))">
           ...
           ${_rowRightControls(round)}
           <span class="ptool-results-toggle">${Icon('chevronDown', 16)}</span>
         </div>
...
function _renderImageGenBlock(round, ctx) {
  ...
  if (imgUri) {
    ...
    const svgBadge = hasSvg
      ...
      : "";
    const svgBtn = svgUrl
      ? `<button class="ig-action-btn" data-tofu-action="event.stopPropagation();_openExternalAsset(this)" data-url="${escapeHtml(svgUrl)}" title="${escapeHtml("Open SVG" + (svgPath ? " — " + svgPath : ""))}">SVG</button>`
      : "";
    const pathBadges = [
      ...
      svgPath ? `<span class="ig-path-chip ig-path-chip-svg" title="SVG saved to project: ${escapeHtml(svgPath)}"><span class="ig-path-icon">${Icon('fileCode', 12)}</span>${escapeHtml(svgPath)}</span>` : "",
    ].filter(Boolean).join("");
    ...
    const imageArea = (isEdit && srcUrl)
      ? `<div class="ig-beforeafter">
               ...
               <span class="ig-ba-arrow" aria-hidden="true">${Icon('arrowRight', 16)}</span>
               <figure class="ig-ba-item">
    ...
    return `<div class="ptool-imagegen-block ${modeCls}" data-rn="${round.roundNum}">
           <div class="ptool-line ptool-imagegen-header">
             ...
             ${paramsBadges}
             ${svgBadge}
             <span class="ptool-badge ptool-badge-ok">${escapeHtml(_plainToolStatus(meta.badge, "done"))}</span>
             ${_rowRightControls(round)}
           </div>
           <div class="imagegen-card">
             ...
             <div class="imagegen-card-footer">
               ...
               <div class="ig-actions">
                 ${svgBtn}
                 <button type="button" class="ig-action-btn" data-tofu-action="event.stopPropagation();_downloadGenImage(this)" title="Download PNG" aria-label="Download PNG">${Icon('download', 16)}</button>
                 <button type="button" class="ig-action-btn" data-tofu-action="event.stopPropagation();_openImageFullscreen(this.closest('.imagegen-card').querySelector('.ig-beforeafter .ig-ba-item:last-child img, img').src)" title="Fullscreen" aria-label="Fullscreen">${Icon('maximize', 16)}</button>
               </div>
             </div>
             ${pathFooter}
           </div>
...
const _GATE_REFUSAL_TOOLS = ["write_file", "edit_file", "apply_diff", "apply_diffs", "insert_content", "insert_contents"];
...
/* Tool-result compaction must stay explicit and precede the tool name:
 * L0 was never in model context, L1 aged out, and L3 became a summary.
 * Solid labels avoid making a model-visibility boundary look decorative. */
...
/* Per-edit operation pill for batch-edit rows. The op names are the API
 * enum values (replace / insert_after / insert_before) — shown verbatim so
 * the row vocabulary matches the header summary "(N edits: replace, …)".
 * The icon carries the at-a-glance signal: ⇄ rewrite vs ↓/↑ pure insertion. */
function _renderOpPill(opName) {
  const _kinds = {
    replace: {
      cls: "ptool-op--replace",
      icon: '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M16 3l4 4-4 4"/><path d="M20 7H4"/><path d="M8 21l-4-4 4-4"/><path d="M4 17h16"/></svg>',
    },
    insert_after: {
      cls: "ptool-op--insert",
      icon: '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="3" x2="12" y2="15"/><polyline points="6 9 12 15 18 9"/><line x1="4" y1="21" x2="20" y2="21"/></svg>',
    },
    insert_before: {
      cls: "ptool-op--insert",
      icon: '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="21" x2="12" y2="9"/><polyline points="6 15 12 9 18 15"/><line x1="4" y1="3" x2="20" y2="3"/></svg>',
    },
  };
  const k = _kinds[opName];
  if (!k) return "";
  return `<span class="ptool-op ${k.cls}" title="operation: ${opName}">${k.icon}${opName}</span>`;
}
...
function _renderBatchEditsBlock(round, ctx, badgeHtml, compactionLabelHtml) {
  const { svg, q, meta, rootPill } = ctx;
  const _isUnifiedEditTool = round.toolName === "edit_file";
  const _isBatchEditTool = _isUnifiedEditTool || round.toolName === "apply_diffs" || round.toolName === "insert_contents";
  if (!(_isBatchEditTool && meta.editSummaries && Array.isArray(meta.editSummaries) &&
        meta.editSummaries.length > (_isUnifiedEditTool ? 0 : 1))) return "";
  ...
  edits.forEach((ed, i) => {
    ...
    const pathHtml = (_multiFile && baseName)
      ...
      : "";
    /* Operation pill. edit_file summaries carry the op server-side
     * (index-aligned by construction); the legacy batch tools imply it
     * from the tool itself — apply_diffs is always a rewrite, and
     * insert_contents derives direction from its position arg. */
    let _opName = typeof ed.operation === "string" ? ed.operation : "";
    if (!_opName || _opName === "?") {
      if (round.toolName === "apply_diffs") _opName = "replace";
      else if (round.toolName === "insert_contents") {
        const _pePos = parsedEdits && parsedEdits[i];
        _opName = (_pePos && _pePos.position === "before") ? "insert_before" : "insert_after";
      }
    }
    const opPillHtml = _renderOpPill(_opName);
    let diffHtml = "";
    if (ed.status !== "fail" && parsedEdits && parsedEdits[i]) {
      const pe = parsedEdits[i];
      const operation = pe.operation || "";
      const isInsert = operation.startsWith("insert_") || (!pe.search && pe.anchor && !operation);
      const oldText = pe.search || pe.anchor || "";
      const newText = isInsert
        ? (((operation === "insert_before" || pe.position === "before") ? (pe.content + "\n") : "") +
           (pe.anchor || "") +
           ((operation !== "insert_before" && pe.position !== "before") ? ("\n" + pe.content) : ""))
        : (operation === "replace" ? (pe.content || "") : (pe.replace || ""));
  ...
  const gateNoticeHtml = _renderGateNotice(_refusalInfo(round, meta));
  /* Collapsed by default (owner directive 2026-08-27): parallel-edit batches
   * no longer fan out into the transcript — the header row already carries
   * the file, edit count and per-op pills; the user expands to audit diffs. */
  return `<details class="ptool-batch-done-block" data-rn="${round.roundNum}">
         <summary class="ptool-line ptool-batch-done-header">
           <span class="ptool-icon">${svg}</span>
           ${compactionLabelHtml}
...
/* Catch-all settled result viewer. JSON is formatted; other payloads remain
 * verbatim so file/grep alignment survives. The visible body is capped by
 * _RESULT_VIEW_MAX and reports truncation to bound repeated DOM rendering. */
const _RESULT_VIEW_MAX = 120000;
function _renderGenericToolResultBlock(round, ctx, badgeHtml, compactionLabelHtml) {
  const { svg, td, q, meta, rootPill, repairedBadge } = ctx;
  if (!round || round.status !== "done") return "";
  let content = typeof round.toolContent === "string" ? round.toolContent : "";
  if (!content.trim() && meta && typeof meta.output === "string") content = meta.output;
  if (!content.trim()) return "";

  const _t = (typeof t === "function") ? t : (k, d) => d;
  let shown = content.replace(/\s+$/, "");
  let isJson = false;
  if (shown.length <= 200000) {
    const c0 = shown.charAt(0);
    if (c0 === "{" || c0 === "[") {
      try { shown = JSON.stringify(JSON.parse(shown), null, 2); isJson = true; }
      catch (_) { /* not JSON after all — render verbatim */ }
    }
  }
  const totalChars = shown.length;
  const truncated = totalChars > _RESULT_VIEW_MAX;
  if (truncated) shown = shown.slice(0, _RESULT_VIEW_MAX);
  const lineCount = shown ? shown.split("\n").length : 0;

  const langLabel = isJson ? "json" : String((td && td.label) || round.toolName || "tool").toLowerCase();
  const stats = _t("tool.resultStats", "{lines} 行 · {chars} 字符")
    .replace("{lines}", lineCount.toLocaleString())
    .replace("{chars}", totalChars.toLocaleString());
  const truncNote = truncated
    ? `<div class="ptool-result-trunc">${escapeHtml(_t("tool.resultTruncated", "结果过长，仅显示前 {n} 字符").replace("{n}", _RESULT_VIEW_MAX.toLocaleString()))}</div>`
    : "";

  return `<details class="ptool-result-block" data-rn="${round.roundNum}">
       ...
       <div class="ptool-result-body">
         <pre class="ptool-result-pre"><div class="code-header"><span>${escapeHtml(langLabel)} · ${escapeHtml(stats)}</span><button class="copy-btn" data-tofu-action="copyCode(this)">Copy</button></div><code>${escapeHtml(shown)}</code></pre>
         ${truncNote}
       </div>
     </details>`;
}
...
function _renderTurnHead(size, rno) {
  ...
  return `<button type="button" class="ptool-turn-head" aria-expanded="true">${rnoHtml}${_turnForkSvg}<span class="ptool-turn-label">${_turnLabelText(size)}</span><span class="ptool-turn-chev">${Icon('chevronDown', 16)}</span></button>`;
}

function _isProgramRound(r) {
  return !!(r && r._programSynthetic);
}
...
function _programDisplayValue(value) {
...
function _renderProgramRound(r) {
  ...
  const result = _programDisplayValue(r.programResult);
...
function _renderToolGroupsHTML(rounds, allRounds, narrByRound) {
  ...
  return _computeToolBatches(rounds).map((g) => {
    ...
    const programs = g.rounds.filter(_isProgramRound);
    const size = g.rounds.length - programs.length;
    const rno = _groupRoundNo(g);
    const head = programs.length ? ""
      : (size >= 2 ? _renderTurnHead(size, rno) : (rno != null ? _renderSoloRoundTag(rno) : ""));
    const narr = (narrByRound && narrByRound.get) ? _renderSegNarrationHTML(narrByRound.get(g.key)) : "";
    return narr + `<div class="ptool-turn${programs.length ? " ptool-program-turn" : ""}" data-llm-round="${escapeHtml(String(g.key))}" data-batch-size="${size}" data-round-no="${rno != null ? rno : ""}">${head}${slots}</div>`;
  }).join("");
}
...
// Shows which memories the bounded local matcher surfaced for this turn.
/**
...
function _memPrefetchSegment(mp) {
  ...
  if (phase === "started") {
    ...
    sub = (mp.totalMemories
      ? _t("memPrefetch.totalN", { n: mp.totalMemories }) + " · "
      : "") + _t("memPrefetch.localLabel");
  } else if (phase === "done") {
    state = "done";
    if (selected === 0) {
      headline = _t("memPrefetch.none");
      sub = `${_t("memPrefetch.candidatesN", { n: candidates || 0 })} · ${_t("memPrefetch.localLabel")}`;
    } else {
      ...
      parts.push(_t("memPrefetch.localLabel"));
...
/* ── My-Context-applied segment for the unified turn-provenance strip ──
...
function _relatedConvsSegment(rc) {
  ...
  if (items.length) {
    const lis = items.map(it => {
      ...
      const titleHtml = id
        ? `<a class="rc-conv-link" href="#" data-tofu-action="event.stopPropagation();try{loadConversation('${id}')}catch(e){};return false;">${title}</a>`
...
/* ── Learned-context segment for the unified turn-provenance strip ──
   The layer-3 consolidation pass AUTO-APPLIES new/reinforced durable context
...
function _prefsLearnedSegment(learned) {
  ...
  const lis = informational.map(p => {
    ...
    const changeId = p.change_id || p.id || "";
    const undo = changeId
      ? `<button class="pl-seg-undo" data-tofu-action="event.stopPropagation();undoContextChange(this,'${escapeHtml(changeId)}')">${escapeHtml(_t("prefs.undo"))}</button>`
      : "";
    return `<li><span class="pl-seg-lead">${escapeHtml(lead)}</span> ${_tpInlineMd(p.summary || "")}${undo}</li>`;
  }).join("");
  const detailHtml =
...
/* ── UNIFIED turn-provenance strip ───────────────────────────────────
   ...
   Called by the typed selector adapter for every Turn phase, so the strip is
   identical while live and after reload. */
function renderTurnProvenanceHtml(msg) {
  ...
  return `<div class="turn-prov ${stripState} tp-expandable" data-tofu-action="this.classList.toggle('tp-expanded')">` +
...
function renderPreferenceLearnedHtml(learned) {
  ...
  const rows = learned.filter(p => p && p.pending).map(p => {
    ...
    return `<div class="pl-row pl-pending" data-pref-id="${pid}">` +
      ...
      `<button class="pl-btn pl-confirm" data-tofu-action="resolvePreference(this,'${pid}',true)">${_t("prefs.confirm")}</button>` +
      `<button class="pl-btn pl-dismiss" data-tofu-action="resolvePreference(this,'${pid}',false)">${_t("prefs.dismiss")}</button>` +
      `</span></div>`;
  }).join("");
  if (!rows) return "";
...
if (typeof window !== 'undefined' && !runtimeScope._cmdTimerTicker) {
  runtimeScope._cmdTimerTicker = setInterval(() => {
    try {
