...
/* ═══════════════════════════════════════════════════════════════════
   ...
   Bundled by Vite's module graph (the migrated module graph). All UI strings live under
...
(function () {
  ...
  var _state = {
    ...
    tab: 'charter',       // active tab (charter|board|activity|peers|status|integration)
  ...
  function _onTabSelected(name, prev) {
    ...
    if (name === 'integration' && prev !== 'integration') {
      _refreshIntegration(_state.path || _displayedProjectPath());
    }
  }
  ...
  function _boardMutate(act, taskId, btn) {
    ...
    if (act === 'complete' && typeof api.boardComplete === 'function') {
      ...
      if (typeof runtimeScope.ProjectBrainAttention !== 'undefined' &&
          runtimeScope.ProjectBrainAttention &&
          typeof runtimeScope.ProjectBrainAttention.focusItem === 'function') {
        runtimeScope.ProjectBrainAttention.focusItem(taskId);
      }
      return;
  ...
  function _renderNoProject() {
    ...
    _setTabCount('pbTabCountIntegration', 0);
  }
  ...
  function _refreshAttention(path) {
    if (typeof runtimeScope.ProjectBrainAttention !== 'undefined' &&
        runtimeScope.ProjectBrainAttention &&
        typeof runtimeScope.ProjectBrainAttention.refreshAttention === 'function') {
      runtimeScope.ProjectBrainAttention.refreshAttention(path);
    }
  }

  /** Drive the Team/Peers column (project-brain-peers.js) if it's loaded. */
  function _refreshPeers(path) {
    if (typeof runtimeScope.ProjectBrainPeers !== 'undefined' &&
        runtimeScope.ProjectBrainPeers &&
        typeof runtimeScope.ProjectBrainPeers.refreshPeers === 'function') {
      runtimeScope.ProjectBrainPeers.refreshPeers(path);
    }
  }

  /** Drive the Status tab (project-brain-status.js) if it's loaded. */
  function _refreshStatus(path) {
    if (typeof runtimeScope.ProjectBrainStatus !== 'undefined' &&
        runtimeScope.ProjectBrainStatus &&
        typeof runtimeScope.ProjectBrainStatus.refreshStatus === 'function') {
      runtimeScope.ProjectBrainStatus.refreshStatus(path);
    }
  }

  /** Drive the deterministic integration tab (no LLM calls). */
  function _refreshIntegration(path) {
    if (typeof runtimeScope.ProjectBrainIntegration !== 'undefined' &&
        runtimeScope.ProjectBrainIntegration &&
        typeof runtimeScope.ProjectBrainIntegration.refreshIntegration === 'function') {
      runtimeScope.ProjectBrainIntegration.refreshIntegration(path);
    }
  }
  ...
  runtimeScope.openProjectBrainInfluence = openProjectBrainInfluence;
  runtimeScope.closeProjectBrain = closeProjectBrain;
  runtimeScope.projectBrainRefresh = projectBrainRefresh;
})();
