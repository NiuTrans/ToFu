/* ===== migrated source: settings/local_endpoints.js ===== */
/* ═══════════════════════════════════════════════════════════════════
...
function addLocalProvider() {
  ...
  for (var i = 0; i < _LOCAL_ENGINE_PRESETS.length; i++) {
    ...
    tiles += '<button class="stg-preset-tile" data-tofu-action="_pickLocalPreset(' + i + ')">' +
  ...
  var html = '<div id="' + id + '" class="stg-modal-overlay" data-tofu-action="if(event.target===this)this.remove()">' +
    '<div class="stg-modal" style="max-width:560px;">' +
      '<div class="stg-modal-header">' +
        ...
        '<button class="stg-modal-close" data-tofu-action="document.getElementById(\'' + id + '\').remove()">✕</button>' +
      '</div>' +
...
function _openBulkEditEndpoints(provIdx) {
  ...
  var html = '<div id="' + existingId + '" class="stg-modal-overlay" data-tofu-action="if(event.target===this)this.remove()">' +
    '<div class="stg-modal" style="max-width:640px;">' +
      '<div class="stg-modal-header">' +
        ...
        '<button class="stg-modal-close" data-tofu-action="document.getElementById(\'' + existingId + '\').remove()">✕</button>' +
      '</div>' +
      '<div class="stg-modal-body">' +
      ...
      '<div class="stg-modal-footer">' +
        '<button class="stg-btn-secondary" data-tofu-action="document.getElementById(\'' + existingId + '\').remove()">' + escapeHtml(t('settings.epBulkCancel')) + '</button>' +
        '<button class="stg-btn-primary" data-tofu-action="_applyBulkEditEndpoints(' + provIdx + ')">' + escapeHtml(t('settings.epBulkApply')) + '</button>' +
      '</div>' +
    '</div>' +
  '</div>';
  document.body.insertAdjacentHTML('beforeend', html);
  ...
  var html = '<div class="stg-ep-item" data-ep-idx="' + epIdx + '">' +
    '<div class="stg-ep-row">' +
      ...
      '<input type="text" data-local-endpoint value="' + escapeHtml(url || '') + '" ' +
        ...
        'data-tofu-action-change="_onLocalEndpointEdit(' + provIdx + ',' + epIdx + ',this.value)">' +
      '<button class="stg-ep-btn" data-tofu-action="_probeLocalEndpoint(' + provIdx + ',' + epIdx + ')" ' +
        'title="' + escapeHtml(t('settings.testEndpointTitle')) + '">↻</button>' +
      '<button class="stg-ep-btn danger" data-tofu-action="_deleteLocalEndpoint(' + provIdx + ',' + epIdx + ')" ' +
  ...
  var html = '<div class="stg-field stg-ep-field">' +
    '<div class="stg-ep-header">' +
      ...
      '<div class="stg-ep-toolbar">' +
        '<button class="stg-btn-add stg-ep-tb" data-tofu-action="_addLocalEndpoint(' + provIdx + ')" ' +
          'title="' + escapeHtml(t('settings.addEndpointTitle')) + '">' + escapeHtml(t('settings.addEndpoint')) + '</button>' +
        '<button class="stg-btn-add stg-ep-tb" data-tofu-action="_openBulkEditEndpoints(' + provIdx + ')" ' +
          'title="' + escapeHtml(t('settings.bulkEditTitle')) + '">' + escapeHtml(t('settings.bulkEdit')) + '</button>' +
        '<button class="stg-btn-add stg-ep-tb" data-tofu-action="_discoverLocalModels(' + provIdx + ')" ' +
          'title="' + escapeHtml(t('settings.probeAllTitle')) + '">' + escapeHtml(t('settings.probeAll')) + '</button>' +
        '<button class="stg-btn-add stg-ep-tb danger" data-tofu-action="_clearLocalEndpoints(' + provIdx + ')" ' +
...
//   models[]: each has { model_id, aliases:[], capabilities:[], rpm,
//                        context_window, pricing:{input,output,currency},
//                        thinking_default }
let _stgProviders = [];
// Typed settings owners live in a separate lazy module and cannot see this
// module's lexical state. Accessor properties preserve the single cache and
// provider list while that compatibility boundary is retired.
Object.defineProperties(runtimeScope, {
  _stgProviders: {
    configurable: true,
    get: function () { return _stgProviders; },
  },
  _keyStatsCache: {
    configurable: true,
    get: function () { return _keyStatsCache; },
    set: function (value) { _keyStatsCache = value; },
  },
  _keyStatsLoading: {
    configurable: true,
    get: function () { return _keyStatsLoading; },
    set: function (value) { _keyStatsLoading = !!value; },
  },
});
