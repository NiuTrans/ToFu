/* ===== migrated source: settings/provider_faces.js ===== */
/* ═══════════════════════════════════════════════════════════════════
   ...
   the same window scope as every other frontend/src/runtime/*.js file.
   ═══════════════════════════════════════════════════════════════════ */
...
async function _refreshFaceResolutions(provIdx) {
  ...
  var payload = {
    ...
    responses_profile: p.responses_profile || 'auto',
    faces: p.faces || {},
    ...
    models: (p.models || []).map(function(m) {
      return {
        ...
        responses_profile: m.responses_profile || '',
...
function _faceChipHTML(provIdx, m) {
  ...
  if (r.protocol === 'responses' && r.responses_profile) {
    title += ' · ' + t('settings.responsesProfileResolved', {
      profile: r.responses_profile,
    });
  }
  if (r.forced) {
  ...
  var html = '<div class="stg-field stg-faces-field" data-prov-idx="' + provIdx + '">' +
    '<div class="stg-faces-header">' +
      ...
      '<button type="button" class="stg-btn-add stg-faces-tb" ' +
        'data-tofu-action="_addFace(' + provIdx + ')" ' +
...
function _renderFaceRow(provIdx, idx, name, baseUrl, protocol) {
   ...
   * and the provider was flipped onto /messages ( S3). */
  ...
  var sel = '<select class="stg-face-proto" data-face-field="protocol" ' +
    ...
    'data-tofu-action-change="_onFaceProtoChange(' + provIdx + ', this)">';
  ...
  return '<div class="stg-face-row" data-face-idx="' + idx + '" ' +
    ...
    '<input type="text" class="stg-face-url" data-face-field="base_url" ' +
      ...
      'data-tofu-action-change="_onFaceRowEdit(' + provIdx + ')">' +
    '<button type="button" class="stg-faces-btn danger" ' +
      'data-tofu-action="_deleteFace(' + provIdx + ',' + idx + ')" ' +
...
async function _deleteFace(provIdx, idx) {
  ...
  for (var i = 0; i < remaining.length; i++) {
    ...
    if (del) del.setAttribute('data-tofu-action', '_deleteFace(' + provIdx + ',' + i + ')');
  }
