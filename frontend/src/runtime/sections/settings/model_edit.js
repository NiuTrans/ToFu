/* ===== migrated source: settings/model_edit.js ===== */
/* ═══════════════════════════════════════════════════════════════════
...
function _addModel(provIdx) {
  ...
  p.models.push({
    model_id: '', aliases: [], capabilities: ['text'], rpm: 30,
    context_window: null,
    thinking_default: false,
    // A model explicitly created by the user may be a private deployment that
    // /models intentionally omits. Never let automatic catalogue retirement
    // delete it.
    catalog_pinned: true
  });
  _renderProvidersTab();
...
function _editModel(provIdx, modelIdx) {
  ...
  var allCaps = ['text', 'vision', 'video', 'thinking', 'cheap', 'image_gen', 'embedding', 'transcription', 'tts', 'audio_chat'];
  ...
  var _authorityCurrency = String(
    (m.pricing && m.pricing.currency) || _eff.currency || 'USD').toUpperCase();
  if (_authorityCurrency !== 'USD' && _authorityCurrency !== 'CNY') {
    _authorityCurrency = 'USD';
  }
  var _displayCurrency = _modelPriceDisplayCurrency(_authorityCurrency);
  var _ovrIn = (m.pricing && m.pricing.input != null)
    ? _modelPriceInputForUi(m.pricing.input, _authorityCurrency) : '';
  var _ovrOut = (m.pricing && m.pricing.output != null)
    ? _modelPriceInputForUi(m.pricing.output, _authorityCurrency) : '';
  var _effIn = _eff.input != null
    ? _modelPriceInputForUi(_eff.input, _eff.currency || 'USD') : '—';
  var _effOut = _eff.output != null
    ? _modelPriceInputForUi(_eff.output, _eff.currency || 'USD') : '—';
  ...
  var html = '<div class="stg-edit-form" data-prov="' + provIdx +
    '" data-model="' + modelIdx + '" data-price-authority-currency="' +
    escapeHtml(_authorityCurrency) + '" data-price-display-currency="' +
    escapeHtml(_displayCurrency) + '">';
  ...
  html += '<div class="stg-edit-sec">' +
    ...
    '<div class="stg-edit-grid">' +
      '<div class="stg-field stg-field-wide"><label>' + escapeHtml(t('settings.meModelId')) + '</label>' +
        '<input type="text" class="stg-edit-mid" value="' + escapeHtml(m.model_id || '') + '" placeholder="' + escapeHtml(t('settings.meModelIdPlaceholder')) + '" spellcheck="false" autocomplete="off" data-tofu-action-input="_onModelIdDraftInput(this)"></div>' +
    '</div>' +
  '</div>';
  ...
  if (typeof _faceNamesFor === 'function') {
    ...
    if (_names.length <= 1) {
      ...
      html += '<div class="stg-field"><label>' + escapeHtml(t('settings.meProvProto')) +
          ...
          '<select class="stg-edit-proto" data-tofu-action-change="_onModelProtoChange(' + provIdx + ', this)">';
      ...
      html += '<div class="stg-field"><label>' + escapeHtml(t('settings.meFace')) +
        ...
        '<select class="stg-edit-face" data-tofu-action-change="_onFacePinChange(this)">' +
  ...
  /* §3 配额与定价 — RPM / context / input / output as a uniform 2×2. The
  ...
  html += '<div class="stg-edit-sec">' +
    ...
    '<div class="stg-edit-grid">' +
      ...
      '<div class="stg-field"><label>' + escapeHtml(t('settings.meContextWindow')) + ' <span class="stg-hint">' + escapeHtml(t('settings.meContextHint')) + '</span></label>' +
        '<input type="number" class="stg-edit-context" value="' + (m.context_window || '') + '" step="1000" min="1" placeholder="—"></div>' +
      '<div class="stg-field"><label>' + escapeHtml(t('settings.meInputPrice')) + ' <span class="stg-hint">' + escapeHtml(t('settings.mePriceHint', { currency: _displayCurrency })) + '</span></label>' +
        '<input type="number" class="stg-edit-pin" value="' + escapeHtml(_ovrIn) + '" step="0.01" min="0" placeholder="' + escapeHtml(_effIn) + '"></div>' +
      '<div class="stg-field"><label>' + escapeHtml(t('settings.meOutputPrice')) + ' <span class="stg-hint">' + escapeHtml(t('settings.mePriceHint', { currency: _displayCurrency })) + '</span></label>' +
        '<input type="number" class="stg-edit-pout" value="' + escapeHtml(_ovrOut) + '" step="0.01" min="0" placeholder="' + escapeHtml(_effOut) + '"></div>' +
    '</div>' +
  '</div>';
  ...
  for (var ci = 0; ci < allCaps.length; ci++) {
    ...
    html += '<button type="button" class="stg-cap-btn' + (active ? ' active' : '') + '" data-cap="' + cap + '" data-tofu-action="this.classList.toggle(\'active\')">' +
  ...
  html += '<input type="text" class="stg-tag-input"' +
    ...
    ' data-tofu-action-keydown="_poolTagKey(this, event)" data-tofu-action-input="_poolTagSplit(this)"' +
    ' data-tofu-action-blur="_poolTagCommit(this)">' +
  '</div></div>';
  ...
  html += '<div class="stg-edit-foot">' +
    ...
    '<div class="stg-edit-actions">' +
      '<button class="stg-btn-secondary" data-tofu-action="this.closest(\'.stg-edit-form\').remove()">' + escapeHtml(t('settings.cancel')) + '</button>' +
      '<button class="stg-btn-primary" data-tofu-action="_saveModelEdit(' + provIdx + ',' + modelIdx + ')">' + escapeHtml(t('settings.apply')) + '</button>' +
    '</div>' +
  '</div>';
  html += '</div>';
...
function _effectiveModelPrices(m) {
  ...
  return {
    input: mp ? mp.input : null,
    output: mp ? mp.output : null,
    currency: mp ? (mp.currency || 'USD') : null,
  };
}
...
function _saveModelEdit(provIdx, modelIdx) {
  ...
  var form = document.querySelector(
    '.stg-edit-form[data-prov="' + provIdx + '"][data-model="' + modelIdx + '"]');
  if (!form) return;
  ...
  var _authorityCurrency = form.getAttribute(
    'data-price-authority-currency') || 'USD';
  var _displayCurrency = form.getAttribute(
    'data-price-display-currency') || 'USD';
  if (_bothValid) {
    _pin = _modelPriceAuthorityFromUi(
      _pin, _displayCurrency, _authorityCurrency);
    _pout = _modelPriceAuthorityFromUi(
      _pout, _displayCurrency, _authorityCurrency);
    _bothValid = (_pin != null && _pout != null);
  }
  if (!_bothEmpty && !_bothValid) {
    showAlert(t('settings.mePriceInvalidWarn'));
    return;
  }
  var _contextRaw = String(form.querySelector('.stg-edit-context').value || '').trim();
  var _context = Number(_contextRaw);
  if (_contextRaw !== '' &&
      (!Number.isInteger(_context) || _context <= 0)) {
    showAlert(t('settings.meContextInvalidWarn'));
    return;
  }
  ...
  if (_contextRaw === '') {
    ...
    delete m.context_window_source;
    delete m.context_window_exact;
  } else {
    m.context_window = _context;
    m.context_window_source = 'operator';
    m.context_window_exact = true;
  }
  ...
  // Dispatch derives its private ordering heuristic from this real rate pair.
  if (_bothValid) {
    var _pr = (m.pricing && typeof m.pricing === 'object' && !Array.isArray(m.pricing)) ? m.pricing : {};
    _pr.input = _pin;
    _pr.output = _pout;
    _pr.currency = _authorityCurrency;
    _pr.unit = 'per_million_tokens';
    m.pricing = _pr;
  } else {
  ...
  // Migration cleanup for a form opened on an old saved configuration.
  delete m.cost;
...
function _poolTagChipHTML(val) {
  return '<span class="stg-tag-chip" data-value="' + escapeHtml(val) + '">' +
    escapeHtml(val) +
    '<span class="stg-tag-x" data-tofu-action="_poolTagRemove(this)">×</span></span>';
}
