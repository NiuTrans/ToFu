...
import { _applyI18n, ready as i18nReady, t } from '../i18n';
...
// ══════════════════════════════════════════════════════════════════
  ...
  function showPrompt(message, options) {
    return Promise.resolve(window.prompt(message, (options && options.defaultValue) || ''));
  }
  ...
  /**
   * Thin wrapper over the unified ESM Api client. Routes
   * every /api/v1/* call through apiTransport.request so this file never raw-fetches
  ...
  async function _api(url, opts) {
    ...
    const r = await apiTransport.request(url, init);
    let body = null;
    ...
    try {
      ...
      block.innerHTML = `
        <div class="settings-row" style="gap:8px; margin-bottom:12px;">
          ...
          <button class="btn btn-primary btn-sm" data-tofu-action="relayAdminCreateUser">${_esc(_t('relayAdmin.create'))}</button>
        </div>
        <table class="settings-table">
          ...
          <tbody>${users.map(u => `
            <tr data-uid="${_esc(u.id)}">
              ...
              <td>
                ${bill ? `<button class="btn btn-secondary btn-sm" data-tofu-action="relayAdminTopup" data-user-id="${_esc(u.id)}">${_esc(_t('relayAdmin.topup'))}</button>` : ''}
                ${bill ? `<button class="btn btn-secondary btn-sm" data-tofu-action="relayAdminViewPayments" data-user-id="${_esc(u.id)}" data-user-email="${_esc(u.email)}">${_esc(_t('relayAdmin.payments'))}</button>` : ''}
                <button class="btn btn-secondary btn-sm" data-tofu-action="relayAdminToggleStatus" data-user-id="${_esc(u.id)}" data-user-status="${_esc(u.status)}">${u.status === 'active' ? _esc(_t('relayAdmin.disable')) : _esc(_t('relayAdmin.enable'))}</button>
              </td>
            </tr>
  ...
  async function relayAdminTopup(element) {
  ...
  async function relayAdminViewPayments(element) {
    const userId = element.dataset.userId;
  ...
  async function relayAdminToggleStatus(element) {
    const userId = element.dataset.userId;
    const currentStatus = element.dataset.userStatus;
  ...
  async function refreshPricing() {
    ...
    try {
      ...
      const _rate = (v, currency) => v == null ? '—' :
        `${Number(v).toFixed(4)} ${_esc(currency || 'USD')}/M`;
      const _roRow = (name, p) => {
        p = p || {};
        if (p.kind === 'tiered') {
          return (p.contextTiers || []).map(t =>
            `<tr><td><code>${_esc(name)}</code><br><small>${_esc(t.id || '')}</small></td>` +
            `<td>${_rate(t.input, t.currency)}</td><td>${_rate(t.output, t.currency)}</td>` +
            `<td>${Number(t.cacheReadMul || 0).toFixed(2)}× input</td>` +
            `<td>${Number(t.cacheWriteMul || 0).toFixed(2)}× input</td>` +
            `<td>≤ ${Number(t.maxPromptTokens || 0).toLocaleString()}</td></tr>`).join('');
        }
        return `<tr><td><code>${_esc(name)}</code></td>` +
          `<td>${_rate(p.input, p.currency)}</td><td>${_rate(p.output, p.currency)}</td>` +
          `<td>${Number(p.cacheReadMul || 0).toFixed(2)}× input</td>` +
          `<td>${Number(p.cacheWriteMul || 0).toFixed(2)}× input</td><td>flat</td></tr>`;
      };
      block.innerHTML = `
        <div class="settings-row" style="gap:8px;margin-bottom:8px;align-items:center;">
          ...
          <button class="btn btn-primary btn-sm" data-tofu-action="relayAdminSaveMargin">${_esc(_t('relayAdmin.saveMargin'))}</button>
        </div>
        ...
        <table class="settings-table">
          <thead><tr>
            ...
            <th>${_esc(_t('relayAdmin.colCacheHit'))}</th><th>${_esc(_t('relayAdmin.colCacheWrite'))}</th><th>Prompt threshold</th>
          </tr></thead>
          <tbody>
    ...
    try {
      ...
      block.innerHTML = `
        <div class="settings-row" style="gap:8px; margin-bottom:12px; flex-wrap:wrap;">
          ...
          <button class="btn btn-primary btn-sm" data-tofu-action="relayAdminMintCodes">${_esc(_t('relayAdmin.generate'))}</button>
        </div>
      ...
      div.innerHTML = `
        <div style="margin-top:8px;padding:10px;background:rgba(80,180,160,0.06);border-left:3px solid var(--accent,#5a8);">
          <div style="margin-bottom:6px;">${_t('relayAdmin.codesGenerated', { n: r.codes.length, amount: _fmtMicro(r.amount_micro) })}
            <button class="btn btn-secondary btn-sm" data-tofu-action="relayAdminCopyCodes">${_esc(_t('relayAdmin.copyAll'))}</button>
          </div>
  ...
  async function relayAdminCopyCodes(element) {
    const text = element.closest('div')?.parentElement?.querySelector('pre')?.textContent || '';
  ...
  // The four panels live on the dedicated /admin page.
  function relayAdminSwitch(elementOrName) {
    const name = typeof elementOrName === 'string'
      ? elementOrName : elementOrName.dataset.pane;
...
export {
  ...
  relayAdminCopyCodes,
  relayAdminCreateUser,
  relayAdminMintCodes,
  relayAdminSaveMargin,
  relayAdminSwitch,
  relayAdminToggleStatus,
  relayAdminTopup,
  relayAdminViewPayments,
};
