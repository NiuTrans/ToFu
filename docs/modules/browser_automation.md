...
| DevTools Bridge | extension CDP broker, `handlers/_devtools.py`, `browser_devtools` |
...
existing ChatUI bridge. This avoids a second browser/session authority and a
...
5. Load-waiting navigation starts bounded CDP Network capture before leaving `about:blank`, then waits for DOM and network stability.
...
12. Settlement verifies claim ownership; timeout/cancel/release closes captures, transfers and ephemeral tabs.
...
- Policy denial/missing write grant: return non-retryable V2 `browser_access_denied` or `browser_write_authorization_required` and settle as error. Arbitrary `browser_execute_js` is always write-classified even when its expression looks read-only; the error names the resolved tab/domain and tells the caller to verify the tab, request the grant, or switch to `browser_read_page`/`browser_research_page`. An origin-relative `fetch('/…')` without `tab_id` fails earlier with retryable `browser_explicit_tab_required`, so the server never guesses a page origin or requests a grant for an unrelated remembered tab. The temporary research tab never silently becomes the working tab.
...
- `download_url_to_server` is the canonical server-location intent; cookie inspection and shell HTTP are never alternate authentication transports.
