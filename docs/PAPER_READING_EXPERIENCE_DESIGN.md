...
>
> **2026-08-06 退役注记**:速览模式(P2)与专注模式(P4)按 owner 指令全链拆除
> (「Just remove the Quick View and Focus buttons」)——按钮/模块文件/CSS/i18n/
> 测试全部移除;下文 §2/§5/§6 中相关行保留为历史记录,以退役标注为准。
...
| 新模块 | 职责 | 关键机制 |
|---|---|---|
| `paper/reading_xp.js` | 体验轨:insight 锚定卡(按锚插到对应节标题后)+ 文库连接可点卡 + 节末检查翻卡(P2)+ 读完回收卡 | 锚卡数据来自 insight 行 meta;回收卡在滚动进度 100% 时显现,内容=thesis+3 要点+连接+1 开放问题 |
...
(速览/专注两模块已于 2026-08-06 按 owner 指令退役——`paper/focus_mode.js`
文件删除,速览折叠逻辑从 reading_xp.js 摘除。)
